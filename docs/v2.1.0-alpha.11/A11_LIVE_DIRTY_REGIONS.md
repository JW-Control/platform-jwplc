...
La prueba física posterior confirmó una mejora drástica de respuesta durante arrastre y edición.

Captura de cierre observada:
...
Durante una prueba anterior del mismo transporte se observó además aproximadamente 20.7 FPS, ACK de 39.7 ms y `563 REGION / 1 FULL / 0 errores`, por lo que el comportamiento fue consistente en sesiones sucesivas.

Conclusión:

```text
RESPONSIVENESS=PASS
DIRTY_REGION_EFFECTIVENESS=PASS
FLOW_CONTROL_ACK=PASS
HEAP_STABILITY=PASS
VISUAL_STABILITY=PASS
LIVE_ERRORS=0
```

No se justifica seguir aumentando `g_frame` ni el baudrate dentro de Alpha11. La configuración actual se congela como baseline del Live Preview de desarrollo.
...
A11_LIVE_FULL_JWH1_COMPATIBILITY=PASS
