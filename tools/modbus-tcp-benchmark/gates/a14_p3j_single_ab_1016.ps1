...
$payloads = @(1016)
$variants = @("INT_COMMIT2", "INT_FUSED")
...
Write-Host "============================================================"
Write-Host " A14 P3J BLOCK - COALESCED RECV COMMIT VS FUSED @ 1016 B / 8x2KB / 26 MHz"
Write-Host "============================================================"
...
Write-Host "SINGLE_VARIABLE=UDP_RX_COALESCED_RECV_COMMIT"
...
$tempRoot = Join-Path $env:TEMP ("jwplc_a14_p3j_udp_rx_commit_ab_{0}" -f $timestamp)
...
Write-Host "P3J_PATCH_COMPOSITION_PREFLIGHT=PASS"
...
foreach ($variant in $variants) {
    ...
    foreach ($payload in $payloads) {
        ...
        for ($run = 1; $run -le $Runs; ++$run) {
            ...
            Write-Host "P3J_RUN VARIANT=$variant PAYLOAD=$payload RUN=$run RUNNER_EXIT=$runExit LOG=$runLog"
            ...
            Write-Host (
                "P3J_RESULT VARIANT={0} PAYLOAD={1} RUN={2} DUT_MBPS={3:F6} PACKETS={4} SERVICE_HOLDS={5} ACTIVE_HOLDS={6} EMPTY_HOLDS={7} EMPTY_HOLD_RATIO_PCT={8:F2} PACKETS_PER_ACTIVE_HOLD_X1000={9} ACTIVE_HOLD_AVG_US={10} ACTIVE_HOLD_MAX_US={11}" -f
                $variant,
                $payload,
                $run,
                $dutMbps,
        ...
        Write-Host (
            "P3J_MEDIAN VARIANT={0} PAYLOAD={1} DUT_MBPS={2:F6} SERVICE_HOLDS={3} EMPTY_HOLD_RATIO_PCT={4:F2} PACKETS_PER_ACTIVE_HOLD_X1000={5} ACTIVE_HOLD_AVG_US={6} ACTIVE_HOLD_MAX_US={7} INT_SKIPS={8} INT_WAKES={9} SPI_READS_PER_PACKET_X1000={10} TOTAL_W5500_READ_CALLS={11}" -f
            $variant,
            $payload,
            $medianDutMbps,
            $medianServiceHolds,
...
Write-Host ""
Write-Host "============================================================"
Write-Host " P3J COMPARATIVE SUMMARY"
Write-Host "============================================================"

foreach ($payload in $payloads) {
    $fused = $results["INT_FUSED|$payload"]
    $commit2 = $results["INT_COMMIT2|$payload"]
    ...
    Write-Host (
        "P3J_SUMMARY PAYLOAD={0} FUSED_MBPS={1:F6} COMMIT2_MBPS={2:F6} THROUGHPUT_DELTA_PCT={3:F2} FUSED_SPI_READS_PER_PACKET_X1000={4} COMMIT2_SPI_READS_PER_PACKET_X1000={5} SPI_READS_REDUCTION_PCT={6:F2} FUSED_TOTAL_READ_CALLS={7} COMMIT2_TOTAL_READ_CALLS={8} TOTAL_READ_CALL_REDUCTION_PCT={9:F2} FUSED_HOLD_AVG_US={10} COMMIT2_HOLD_AVG_US={11} FUSED_HOLD_MAX_US={12} COMMIT2_HOLD_MAX_US={13}" -f
        $payload,
        $fused.DutMbps,
        $commit2.DutMbps,
        $throughputDeltaPct,
...
$commit21016 = $results["INT_COMMIT2|1016"]
...
$interpretation = "COMMIT2_NO_CLEAR_BENEFIT"

if ($gain1016 -ge 1.0) {
    $interpretation = "COMMIT2_THROUGHPUT_GAIN"
}
elseif (
    $commit21016.DutMbps -ge ($fused1016.DutMbps * 0.995) -and
    $readsReduction1016 -ge 8.0
) {
    $interpretation = "COMMIT2_SPI_EFFICIENCY_GAIN"
}

Write-Host ""
Write-Host ("P3J_1016_THROUGHPUT_DELTA_PCT={0:F2}" -f $gain1016)
Write-Host ("P3J_1016_SPI_READ_REDUCTION_PCT={0:F2}" -f $readsReduction1016)
Write-Host "P3J_INTERPRETATION=$interpretation"
...
Write-Host ""
Write-Host "A14_P3J_SPI_HZ=26000000"
Write-Host "A14_P3J_SOCKET_TOPOLOGY=8x2KB_UNCHANGED"
Write-Host "A14_P3J_ALGORITHM=UDP_RX_BATCH2_INT_AND_FUSED_FROZEN"
Write-Host "A14_P3J_ETH_INT_PIN=GPIO15"
Write-Host "A14_P3J_ISR_SPI_ACCESS=NO"
Write-Host "A14_P3J_PRODUCT_SOURCE_MUTATION=NO"
Write-Host "A14_P3J_DIAGNOSTIC_COPY_ONLY=YES"
Write-Host "A14_P3J_PER_RUN_SERIAL_RESET_GUARD=YES"
Write-Host "A14_P3J_BLOCK_COALESCED_COMMIT_AB=PASS"
Write-Host "NEXT_ACTION=P3J_REPEATABILITY_AGGREGATION"
