...
Write-Host "============================================================"
Write-Host " A14 RTU-H3C - STRUCTURAL SERVER FRAMING + BULK RX TCP500"
Write-Host "============================================================"
...
$runner = Get-G2Path "tools/modbus-tcp-benchmark/pc/a14_rtuh3c_structural_server_longrun.py"
...
foreach ($required in @($archivePath,$rs485Header,$rs485Cpp,$rtuHeader,$rtuCpp,$masterSketch,$slaveSketch,$runner,$p5bGate)) {
    if (-not (Test-Path -LiteralPath $required)) { throw "RTUH3C_REQUIRED_PATH_MISSING=$required" }
}
if ($DurationS -lt 600.0) { throw "RTUH3C_DURATION_MUST_BE_AT_LEAST_600S" }
if ($BucketSeconds -le 0.0) { throw "RTUH3C_BUCKET_SECONDS_INVALID" }

$dirty = @(Get-G2TrackedDirtyPaths)
$staged = @(& git -C $script:G2RepoRoot diff --cached --name-only)
if ($dirty.Count -ne 1 -or $dirty[0].Replace("\", "/") -ne $script:G2CoreRelative) {
    $dirty | ForEach-Object { Write-Host "DIRTY=$_" }
    throw "RTUH3C_EXPECTED_ONLY_DIRTY_CORE_A"
}
if ($staged.Count -ne 0) { throw "RTUH3C_INDEX_NOT_CLEAN" }

$coreHash = Get-G2Sha256 $script:G2CoreRelative
$archiveHashBefore = Get-G2Sha256 $archiveRelative
if ($coreHash -ne $expectedCoreHash) { throw "RTUH3C_UNEXPECTED_CORE_HASH" }
if ($archiveHashBefore -ne $expectedArchiveHash) { throw "RTUH3C_UNEXPECTED_ARCHIVE_HASH" }
...
$checks = @(
    ...
    [PSCustomObject]@{ Label="H3C_TXN_LATENCY"; Pass=$masterText.Contains('Serial.print("RTU_TRANSACTION_MAX_US=")') -and $masterText.Contains('Serial.print("RTU_TRANSACTIONS_OVER_20MS=")') },
    [PSCustomObject]@{ Label="H3C_FAILURE_LATENCY"; Pass=$masterText.Contains('Serial.print("RTU_LAST_FAILURE_DURATION_US=")') -and $masterText.Contains('Serial.print("RTU_LAST_FAILURE_RESULT=")') },
    [PSCustomObject]@{ Label="H3C_STATS_RX_BYTES"; Pass=$rtuHeaderText.Contains("uint64_t rxBytes;") -and $rtuCppText.Contains("_stats.rxBytes") },
    [PSCustomObject]@{ Label="H3C_STATS_TX_BYTES"; Pass=$rtuHeaderText.Contains("uint64_t txBytes;") -and $rtuCppText.Contains("_stats.txBytes") },
    [PSCustomObject]@{ Label="H3C_SERVER_DISCARDS"; Pass=$rtuHeaderText.Contains("serverDiscardedTails") -and $rtuHeaderText.Contains("serverDiscardedBytes") -and $rtuCppText.Contains("_stats.serverDiscardedTails++") },
    [PSCustomObject]@{ Label="H3C_MASTER_SNAPSHOT"; Pass=$masterText.Contains('Serial.print("RTU_RX_BYTES=")') -and $masterText.Contains('Serial.print("RTU_TX_BYTES=")') },
    [PSCustomObject]@{ Label="H3C_SLAVE_SNAPSHOT"; Pass=$slaveText.Contains('Serial.print("RTU_SERVER_DISCARDED_TAILS=")') -and $slaveText.Contains('Serial.print("RTU_SERVER_DISCARDED_BYTES=")') },
    [PSCustomObject]@{ Label="H3C_API"; Pass=$rtuHeaderText.Contains("setEarlyServerDispatchEnabled") -and $rtuHeaderText.Contains("earlyServerDispatchEnabled") },
    [PSCustomObject]@{ Label="H3C_DEFAULT_FALSE"; Pass=$rtuCppText.Contains("_earlyServerDispatchEnabled(false)") },
    [PSCustomObject]@{ Label="H3C_PARTIAL_HOLD"; Pass=$rtuCppText.Contains("JWPLC_MODBUS_FAST_PARTIAL_HOLD_US = 1750UL") -and $rtuCppText.Contains("preservePartial") },
    [PSCustomObject]@{ Label="H3C_EARLY_PARSE"; Pass=$rtuCppText.Contains("if (_rxLength >= expectedLength)") -and $rtuCppText.Contains("parseNow = true") },
    [PSCustomObject]@{ Label="H3C_MASTER_COMMAND"; Pass=$masterText.Contains('RTU_SERVER_FRAMING=STRUCTURAL') -and $masterText.Contains('RTU_SERVER_FRAMING=GAP') },
    [PSCustomObject]@{ Label="H3C_SLAVE_COMMAND"; Pass=$slaveText.Contains('RTU_SERVER_FRAMING=STRUCTURAL') -and $slaveText.Contains('RTU_SERVER_FRAMING=GAP') }
)
foreach ($check in $checks) {
    Write-Host "$($check.Label)=$($check.Pass)"
    if (-not $check.Pass) { throw "RTUH3C_SOURCE_CONTRACT_FAILED_$($check.Label)" }
}
Write-Host "RTUH3C_SOURCE_CONTRACT=PASS"

$pythonCommand = Get-Command python.exe -ErrorAction SilentlyContinue
if ($null -eq $pythonCommand) { $pythonCommand = Get-Command python -ErrorAction SilentlyContinue }
if ($null -eq $pythonCommand) { throw "RTUH3C_PYTHON_NOT_FOUND" }
$pythonExe = $pythonCommand.Source
& $pythonExe -m py_compile $runner
if ($LASTEXITCODE -ne 0) { throw "RTUH3C_PYTHON_SYNTAX_FAILED" }
Write-Host "RTUH3C_PYTHON_SYNTAX=PASS"

$tempRoot = Join-Path $env:TEMP ("jwplc_a14_rtuh3c_structural_{0}" -f (Get-Date -Format "yyyyMMdd_HHmmss"))
$archiveBackup = Join-Path $tempRoot "libJWPLC_ModbusRTU.before.a"
$setupLog = Join-Path $tempRoot "setup_source.log"
$runLog = Join-Path $tempRoot "rtuh3c_structural.log"
...
try {
    ...
    $archiveHidden = $true
    Write-Host ""
    Write-Host "=== FORCE FRESH SOURCE COMPILE / UPLOAD H3C ==="
    ...
    Write-Host "RTUH3C_SETUP_EXIT=$setupExit"
    Get-Content -LiteralPath $setupLog | ForEach-Object { Write-Host $_ }
    if ($setupExit -ne 0) { throw "RTUH3C_SETUP_FAILED" }
    ...
    if (-not $tempRootMatch.Success -or -not $ipMatch.Success) { throw "RTUH3C_SETUP_METADATA_MISSING" }
    ...
    foreach ($buildName in @("build_master","build_slave")) {
        ...
        foreach ($objectPattern in @("JWPLC_ModbusRTU.cpp.o*","JWPLC_RS485.cpp.o*")) {
            ...
            if ($objects.Count -lt 1) { throw ("RTUH3C_SOURCE_OBJECT_MISSING={0}:{1}" -f $buildPath,$objectPattern) }
            Write-Host ("RTUH3C_SOURCE_OBJECT={0}" -f $objects[0].FullName)
        }
    }
    Write-Host "RTUH3C_FRESH_SOURCE_COMPILE=PASS"
}
finally {
...
if ($archiveHashAfter -ne $archiveHashBefore) { throw "RTUH3C_ARCHIVE_RESTORE_HASH_MISMATCH" }
...
if ($dirtyAfterRestore.Count -ne 1 -or $dirtyAfterRestore[0].Replace("\", "/") -ne $script:G2CoreRelative) {
    ...
    throw "RTUH3C_DIRTY_SCOPE_AFTER_RESTORE_INVALID"
}

Write-Host ""
Write-Host "=== PHYSICAL RTU-H3C STRUCTURAL LONG-RUN ==="
...
Write-Host "RTUH3C_RUNNER_EXIT=$runExit"
Write-Host "RTUH3C_RUNNER_LOG=$runLog"
if ($runExit -ne 0) { throw "RTUH3C_RUN_FAILED" }
...
if (-not ($masterPhysical -and $slavePhysical)) { throw "RTUH3C_TFT_PHYSICAL_REVIEW" }
...
if ($finalCoreHash -ne $expectedCoreHash) { throw "RTUH3C_CORE_HASH_CHANGED" }
if ($finalArchiveHash -ne $archiveHashBefore) { throw "RTUH3C_ARCHIVE_CHANGED" }
if ($finalDirty.Count -ne 1 -or $finalDirty[0].Replace("\", "/") -ne $script:G2CoreRelative) { throw "RTUH3C_FINAL_DIRTY_SCOPE_INVALID" }
if ($finalStaged.Count -ne 0) { throw "RTUH3C_FINAL_INDEX_NOT_CLEAN" }

Write-Host "A14_RTU_H3C_STRUCTURAL_LONGRUN_GATE=PASS"
Write-Host "NEXT=RETURN_OUTPUT_TO_CHAT_FOR_STRUCTURAL_FRAMING_DECISION"
