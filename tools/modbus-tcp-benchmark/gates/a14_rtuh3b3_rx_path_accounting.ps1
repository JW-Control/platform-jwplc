...
Write-Host "============================================================"
Write-Host " A14 RTU-H3B.3 - RX-PATH ACCOUNTING + BULK RX TCP500"
Write-Host "============================================================"
...
$runner = Get-G2Path "tools/modbus-tcp-benchmark/pc/a14_rtuh3b3_rx_path_accounting.py"
...
foreach ($required in @($archivePath,$rs485Header,$rs485Cpp,$rtuHeader,$rtuCpp,$masterSketch,$slaveSketch,$runner,$p5bGate)) {
    if (-not (Test-Path -LiteralPath $required)) { throw "RTUH3B3_REQUIRED_PATH_MISSING=$required" }
}
if ($DurationS -lt 600.0) { throw "RTUH3B3_DURATION_MUST_BE_AT_LEAST_600S" }
if ($BucketSeconds -le 0.0) { throw "RTUH3B3_BUCKET_SECONDS_INVALID" }

$dirty = @(Get-G2TrackedDirtyPaths)
$staged = @(& git -C $script:G2RepoRoot diff --cached --name-only)
if ($dirty.Count -ne 1 -or $dirty[0].Replace("\", "/") -ne $script:G2CoreRelative) {
    $dirty | ForEach-Object { Write-Host "DIRTY=$_" }
    throw "RTUH3B3_EXPECTED_ONLY_DIRTY_CORE_A"
}
if ($staged.Count -ne 0) { throw "RTUH3B3_INDEX_NOT_CLEAN" }

$coreHash = Get-G2Sha256 $script:G2CoreRelative
$archiveHashBefore = Get-G2Sha256 $archiveRelative
if ($coreHash -ne $expectedCoreHash) { throw "RTUH3B3_UNEXPECTED_CORE_HASH" }
if ($archiveHashBefore -ne $expectedArchiveHash) { throw "RTUH3B3_UNEXPECTED_ARCHIVE_HASH" }
...
Write-Host "RTU_DIAG_FLOOR_HZ=650"
...
$checks = @(
    ...
    [PSCustomObject]@{ Label="H3B3_TXN_LATENCY"; Pass=$masterText.Contains('Serial.print("RTU_TRANSACTION_MAX_US=")') -and $masterText.Contains('Serial.print("RTU_TRANSACTIONS_OVER_20MS=")') },
    [PSCustomObject]@{ Label="H3B3_FAILURE_LATENCY"; Pass=$masterText.Contains('Serial.print("RTU_LAST_FAILURE_DURATION_US=")') -and $masterText.Contains('Serial.print("RTU_LAST_FAILURE_RESULT=")') },
    [PSCustomObject]@{ Label="H3B3_STATS_RX_BYTES"; Pass=$rtuHeaderText.Contains("uint64_t rxBytes;") -and $rtuCppText.Contains("_stats.rxBytes") },
    [PSCustomObject]@{ Label="H3B3_STATS_TX_BYTES"; Pass=$rtuHeaderText.Contains("uint64_t txBytes;") -and $rtuCppText.Contains("_stats.txBytes") },
    [PSCustomObject]@{ Label="H3B3_SERVER_DISCARDS"; Pass=$rtuHeaderText.Contains("serverDiscardedTails") -and $rtuHeaderText.Contains("serverDiscardedBytes") -and $rtuCppText.Contains("_stats.serverDiscardedTails++") },
    [PSCustomObject]@{ Label="H3B3_MASTER_SNAPSHOT"; Pass=$masterText.Contains('Serial.print("RTU_RX_BYTES=")') -and $masterText.Contains('Serial.print("RTU_TX_BYTES=")') },
    [PSCustomObject]@{ Label="H3B3_SLAVE_SNAPSHOT"; Pass=$slaveText.Contains('Serial.print("RTU_SERVER_DISCARDED_TAILS=")') -and $slaveText.Contains('Serial.print("RTU_SERVER_DISCARDED_BYTES=")') }
)
foreach ($check in $checks) {
    Write-Host "$($check.Label)=$($check.Pass)"
    if (-not $check.Pass) { throw "RTUH3B3_SOURCE_CONTRACT_FAILED_$($check.Label)" }
}
Write-Host "RTUH3B3_SOURCE_CONTRACT=PASS"

$pythonCommand = Get-Command python.exe -ErrorAction SilentlyContinue
if ($null -eq $pythonCommand) { $pythonCommand = Get-Command python -ErrorAction SilentlyContinue }
if ($null -eq $pythonCommand) { throw "RTUH3B3_PYTHON_NOT_FOUND" }
$pythonExe = $pythonCommand.Source
& $pythonExe -m py_compile $runner
if ($LASTEXITCODE -ne 0) { throw "RTUH3B3_PYTHON_SYNTAX_FAILED" }
Write-Host "RTUH3B3_PYTHON_SYNTAX=PASS"

$tempRoot = Join-Path $env:TEMP ("jwplc_a14_rtuh3b3_rxpath_{0}" -f (Get-Date -Format "yyyyMMdd_HHmmss"))
$archiveBackup = Join-Path $tempRoot "libJWPLC_ModbusRTU.before.a"
$setupLog = Join-Path $tempRoot "setup_source.log"
$runLog = Join-Path $tempRoot "rtuh3b3_rxpath.log"
...
try {
    ...
    $archiveHidden = $true
    Write-Host ""
    Write-Host "=== FORCE FRESH SOURCE COMPILE / UPLOAD H3B3 ==="
    ...
    Write-Host "RTUH3B3_SETUP_EXIT=$setupExit"
    Get-Content -LiteralPath $setupLog | ForEach-Object { Write-Host $_ }
    if ($setupExit -ne 0) { throw "RTUH3B3_SETUP_FAILED" }
    ...
    if (-not $tempRootMatch.Success -or -not $ipMatch.Success) { throw "RTUH3B3_SETUP_METADATA_MISSING" }
    ...
    foreach ($buildName in @("build_master","build_slave")) {
        ...
        foreach ($objectPattern in @("JWPLC_ModbusRTU.cpp.o*","JWPLC_RS485.cpp.o*")) {
            ...
            if ($objects.Count -lt 1) { throw ("RTUH3B3_SOURCE_OBJECT_MISSING={0}:{1}" -f $buildPath,$objectPattern) }
            Write-Host ("RTUH3B3_SOURCE_OBJECT={0}" -f $objects[0].FullName)
        }
    }
    Write-Host "RTUH3B3_FRESH_SOURCE_COMPILE=PASS"
}
finally {
...
if ($archiveHashAfter -ne $archiveHashBefore) { throw "RTUH3B3_ARCHIVE_RESTORE_HASH_MISMATCH" }
...
if ($dirtyAfterRestore.Count -ne 1 -or $dirtyAfterRestore[0].Replace("\", "/") -ne $script:G2CoreRelative) {
    ...
    throw "RTUH3B3_DIRTY_SCOPE_AFTER_RESTORE_INVALID"
}

Write-Host ""
Write-Host "=== PHYSICAL RTU-H3B3 RX-PATH DIAGNOSTIC ==="
...
Write-Host "RTUH3B3_RUNNER_EXIT=$runExit"
Write-Host "RTUH3B3_RUNNER_LOG=$runLog"
if ($runExit -ne 0) { throw "RTUH3B3_RUN_FAILED" }
...
if (-not ($masterPhysical -and $slavePhysical)) { throw "RTUH3B3_TFT_PHYSICAL_REVIEW" }
...
if ($finalCoreHash -ne $expectedCoreHash) { throw "RTUH3B3_CORE_HASH_CHANGED" }
if ($finalArchiveHash -ne $archiveHashBefore) { throw "RTUH3B3_ARCHIVE_CHANGED" }
if ($finalDirty.Count -ne 1 -or $finalDirty[0].Replace("\", "/") -ne $script:G2CoreRelative) { throw "RTUH3B3_FINAL_DIRTY_SCOPE_INVALID" }
if ($finalStaged.Count -ne 0) { throw "RTUH3B3_FINAL_INDEX_NOT_CLEAN" }

Write-Host "A14_RTU_H3B3_RX_PATH_ACCOUNTING_GATE=PASS"
Write-Host "NEXT=RETURN_OUTPUT_TO_CHAT_FOR_RX_PATH_ROOT_CAUSE_DECISION"
