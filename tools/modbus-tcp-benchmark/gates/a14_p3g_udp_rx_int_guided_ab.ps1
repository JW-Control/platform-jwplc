...
Write-Host "============================================================"
Write-Host " A14 P3G - UDP RX POLLING VS W5500 INT @ 8x2KB / 26 MHz"
Write-Host "============================================================"
...
$tempRoot = Join-Path $env:TEMP ("jwplc_a14_p3g_udp_rx_int_ab_{0}" -f $timestamp)
...
foreach ($variant in $variants) {
    ...
    if ($variant -eq "INT_GUIDED") {
        ...
        if ($intExit -ne 0) {
            ...
            throw "P3G_INT_PATCH_FAILED"
        }

        Get-Content -LiteralPath $intLog | ForEach-Object { Write-Host $_ }
    }
    else {
        Write-Host "INT_PATCH=NOT_APPLIED"
    }
    ...
    foreach ($payload in $payloads) {
        ...
        for ($run = 1; $run -le $Runs; ++$run) {
            ...
            Write-Host "P3G_RUN VARIANT=$variant PAYLOAD=$payload RUN=$run RUNNER_EXIT=$runExit LOG=$runLog"
            ...
            if ($variant -eq "INT_GUIDED") {
                if ((Get-LogValue -Text $text -Key "P3G_FINAL_ETH_INT_CONFIGURED") -ne "YES") {
                    throw "P3G_INT_NOT_CONFIGURED_P$payload" + "_R$run"
                }
                ...
                if ($intPin -ne 15) {
                    throw "P3G_INT_PIN_MISMATCH=$intPin"
                }

                if ($intSocket -ne 0) {
                    throw "P3G_EXPECTED_UDP_SOCKET0_ACTUAL=$intSocket"
                }

                if ($skipCount -le 0 -or $wakeCount -le 0) {
                    throw "P3G_INT_ACTIVITY_MISSING_P$payload" + "_R$run"
                }
            ...
            Write-Host (
                "P3G_RESULT VARIANT={0} PAYLOAD={1} RUN={2} DUT_MBPS={3:F6} PACKETS={4} SERVICE_HOLDS={5} ACTIVE_HOLDS={6} EMPTY_HOLDS={7} EMPTY_HOLD_RATIO_PCT={8:F2} PACKETS_PER_ACTIVE_HOLD_X1000={9} ACTIVE_HOLD_AVG_US={10} ACTIVE_HOLD_MAX_US={11}" -f
                $variant,
                $payload,
                $run,
                $dutMbps,
        ...
        Write-Host (
            "P3G_MEDIAN VARIANT={0} PAYLOAD={1} DUT_MBPS={2:F6} SERVICE_HOLDS={3} EMPTY_HOLD_RATIO_PCT={4:F2} PACKETS_PER_ACTIVE_HOLD_X1000={5} ACTIVE_HOLD_AVG_US={6} ACTIVE_HOLD_MAX_US={7} INT_SKIPS={8} INT_WAKES={9}" -f
            $variant,
            $payload,
            $medianDutMbps,
            $medianServiceHolds,
...
Write-Host ""
Write-Host "============================================================"
Write-Host " P3G COMPARATIVE SUMMARY"
Write-Host "============================================================"

foreach ($payload in $payloads) {
    $poll = $results["POLLING|$payload"]
    $intr = $results["INT_GUIDED|$payload"]

    $throughputDeltaPct =
        (($intr.DutMbps / $poll.DutMbps) - 1.0) * 100.0

    $serviceHoldReductionPct =
        (1.0 - ($intr.ServiceHolds / [double]$poll.ServiceHolds)) * 100.0

    $emptyRatioDeltaPct =
        $intr.EmptyHoldRatioPct - $poll.EmptyHoldRatioPct

    Write-Host (
        "P3G_SUMMARY PAYLOAD={0} POLLING_MBPS={1:F6} INT_MBPS={2:F6} THROUGHPUT_DELTA_PCT={3:F2} POLLING_SERVICE_HOLDS={4} INT_SERVICE_HOLDS={5} SERVICE_HOLD_REDUCTION_PCT={6:F2} POLLING_EMPTY_RATIO_PCT={7:F2} INT_EMPTY_RATIO_PCT={8:F2} EMPTY_RATIO_DELTA_PCT={9:F2} INT_SKIPS={10} INT_WAKES={11}" -f
        $payload,
        $poll.DutMbps,
        $intr.DutMbps,
        $throughputDeltaPct,
        ...
        $intr.ServiceHolds,
        $serviceHoldReductionPct,
        $poll.EmptyHoldRatioPct,
        $intr.EmptyHoldRatioPct,
        $emptyRatioDeltaPct,
        $intr.IntSkips,
        $intr.IntWakes
    )
...
$poll1016 = $results["POLLING|1016"]
$int1016 = $results["INT_GUIDED|1016"]

$gain1016 =
    (($int1016.DutMbps / $poll1016.DutMbps) - 1.0) * 100.0

$holdReduction1016 =
    (1.0 - ($int1016.ServiceHolds / [double]$poll1016.ServiceHolds)) * 100.0

$interpretation = "INT_NO_CLEAR_BENEFIT"

if (
    $int1016.DutMbps -ge ($poll1016.DutMbps * 0.98) -and
    $holdReduction1016 -ge 30.0
) {
    $interpretation = "INT_REDUCES_SPI_POLLING_WITHOUT_MATERIAL_THROUGHPUT_LOSS"
}
...
if (
    ...
    $interpretation = "INT_IMPROVES_THROUGHPUT_AND_REDUCES_SPI_POLLING"
}

Write-Host ""
Write-Host ("P3G_1016_THROUGHPUT_DELTA_PCT={0:F2}" -f $gain1016)
Write-Host ("P3G_1016_SERVICE_HOLD_REDUCTION_PCT={0:F2}" -f $holdReduction1016)
Write-Host "P3G_INTERPRETATION=$interpretation"
...
Write-Host ""
Write-Host "A14_P3G_SPI_HZ=26000000"
Write-Host "A14_P3G_SOCKET_TOPOLOGY=8x2KB_UNCHANGED"
Write-Host "A14_P3G_ALGORITHM=UDP_RX_BATCH2_FROZEN"
Write-Host "A14_P3G_ETH_INT_PIN=GPIO15"
Write-Host "A14_P3G_ISR_SPI_ACCESS=NO"
Write-Host "A14_P3G_PRODUCT_SOURCE_MUTATION=NO"
Write-Host "A14_P3G_DIAGNOSTIC_COPY_ONLY=YES"
Write-Host "A14_P3G_UDP_RX_INT_GUIDED_AB=PASS"
Write-Host "NEXT_ACTION=RETURN_OUTPUT_TO_CHAT_FOR_INT_DECISION"
