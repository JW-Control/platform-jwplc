        ...
        try:
            ...
            while not self.stop_event.is_set():
                ...
                if host == VALID_HOST:
                    ...
                    sock.sendto(build_a_response(packet, question_end), peer)
...
def main():
    ...
    while time.monotonic() < deadline:
        if any(line.startswith("NB2_DNS_PROBE_READY=YES") for line in lines):
            break
        time.sleep(0.05)
    else:
        print("NB2_DNS_READY_TIMEOUT=YES", flush=True)
        stop_serial.set()
    ...
    ser.flush()
    print("DNS_RUN_COMMAND_SENT=YES", flush=True)

    if not result_end.wait(timeout=max(1.0, deadline - time.monotonic())):
        print("NB2_DNS_RESULT_TIMEOUT=YES", flush=True)
        stop_serial.set()
        reader.join(timeout=1.0)
        ser.close()
        dns.stop()
    ...
    print(f"DNS_VALID_QUERY_COUNT={dns.valid_queries}", flush=True)
    print(f"DNS_TIMEOUT_QUERY_COUNT={dns.timeout_queries}", flush=True)
    print(f"DNS_OTHER_QUERY_COUNT={dns.other_queries}", flush=True)
    if dns.error is not None:
        print(f"DNS_SERVER_ERROR={dns.error}", flush=True)
        return 6
    print("NB2_DNS_CLIENT_PASS=YES", flush=True)
    return 0
