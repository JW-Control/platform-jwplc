...
$batch2MedianDutMbps = [double]11.335577
$batch2MedianActiveHoldAvgUs = [int64]862
$batch2MedianActiveHoldMaxUs = [int64]2251
...
Write-Host "============================================================"
Write-Host " A14 P3C - UDP RX SPLIT2 REACQUIRE A/B @ 26 MHz"
Write-Host "============================================================"
...
Write-Host "ALGORITHM_CHANGE=UDP_RX_TWO_ATTEMPTS_PER_LOOP_WITH_SPI_REACQUIRE"
...
Write-Host "BATCH2_MEDIAN_UDP_RX_DUT_MBPS=$batch2MedianDutMbps"
Write-Host "BATCH2_MEDIAN_ACTIVE_HOLD_AVG_US=$batch2MedianActiveHoldAvgUs"
Write-Host "BATCH2_MEDIAN_ACTIVE_HOLD_MAX_US=$batch2MedianActiveHoldMaxUs"
...
$splitPatchPath = Join-Path $PSScriptRoot "a14_p3c_udp_rx_split2_reacquire_patch.py"
...
$tempRoot = Join-Path $env:TEMP ("jwplc_a14_p3c_udp_rx_split2_{0}" -f $timestamp)
...
Write-Host ""
Write-Host "=== APPLY SINGLE-VARIABLE UDP RX SPLIT2/REACQUIRE PATCH ==="

$diagSketchPath = Join-Path $workRoot "sketch\eth14_raw_transport_server\eth14_raw_transport_server.ino"
$splitPatchLog = Join-Path $tempRoot "split2_patch.log"
$splitPatchArgs = @(
    $splitPatchPath,
    "--instrumented-sketch", $diagSketchPath
)
$splitPatchExit = Invoke-NativeToLog -FilePath $pythonExe -Arguments $splitPatchArgs -LogPath $splitPatchLog

Write-Host "SPLIT2_PATCH_EXIT=$splitPatchExit"
Write-Host "SPLIT2_PATCH_LOG=$splitPatchLog"

if ($splitPatchExit -ne 0) {
    Get-Content -LiteralPath $splitPatchLog -Tail 120 | ForEach-Object { Write-Host $_ }
    throw "P3C_SPLIT2_PATCH_FAILED"
}

Get-Content -LiteralPath $splitPatchLog | ForEach-Object { Write-Host $_ }
...
Write-Host ""
Write-Host "=== UDP RX SPLIT2/REACQUIRE A/B RUNS ==="
...
for ($run = 1; $run -le $Runs; ++$run) {
    ...
    if ($activeHolds -le 0) { throw "P3C_ACTIVE_HOLD_COUNT_EMPTY_$run" }
    if ($activeHolds -ne $packets) { throw "P3C_ACTIVE_HOLD_PACKET_MISMATCH_$run" }
    ...
    if ($packetsPerActiveHoldX1000 -ne 1000) {
        throw "P3C_PACKETS_PER_LOCK_NOT_ONE_$run=$packetsPerActiveHoldX1000"
    }
...
$batch2GainPct = (($batch2MedianDutMbps / $baselineMedianDutMbps) - 1.0) * 100.0
...
$gainRetainedPct = 0.0
if ($batch2MedianDutMbps -gt $baselineMedianDutMbps) {
    $gainRetainedPct =
        (($medianDutMbps - $baselineMedianDutMbps) /
        ($batch2MedianDutMbps - $baselineMedianDutMbps)) * 100.0
}

$p3cInterpretation = "MIXED"
if ($gainRetainedPct -ge 80.0) {
    $p3cInterpretation = "PACING_OR_EXTRA_ATTEMPT_DOMINANT"
}
elseif ($gainRetainedPct -le 20.0) {
    $p3cInterpretation = "SAME_LOCK_BATCHING_DOMINANT"
}

Write-Host ""
Write-Host "=== P3C SPLIT2 VS BASELINE / BATCH2 ==="
Write-Host ("P3C_BASELINE_MEDIAN_DUT_MBPS={0:F6}" -f $baselineMedianDutMbps)
Write-Host ("P3C_BATCH2_MEDIAN_DUT_MBPS={0:F6}" -f $batch2MedianDutMbps)
Write-Host ("P3C_SPLIT2_MEDIAN_DUT_MBPS={0:F6}" -f $medianDutMbps)
Write-Host ("P3C_SPLIT2_DELTA_VS_BASELINE_PCT={0:F2}" -f $throughputGainPct)
Write-Host ("P3C_BATCH2_DELTA_VS_BASELINE_PCT={0:F2}" -f $batch2GainPct)
Write-Host ("P3C_BATCH2_GAIN_RETAINED_PCT={0:F2}" -f $gainRetainedPct)
Write-Host "P3C_BASELINE_ACTIVE_HOLD_AVG_US=$baselineMedianActiveHoldAvgUs"
Write-Host "P3C_BATCH2_ACTIVE_HOLD_AVG_US=$batch2MedianActiveHoldAvgUs"
Write-Host "P3C_SPLIT2_ACTIVE_HOLD_AVG_US=$medianActiveHoldAvg"
Write-Host ("P3C_SPLIT2_ACTIVE_HOLD_AVG_DELTA_VS_BASELINE_PCT={0:F2}" -f $holdAvgGainPct)
Write-Host "P3C_BASELINE_ACTIVE_HOLD_MAX_US=$baselineMedianActiveHoldMaxUs"
Write-Host "P3C_BATCH2_ACTIVE_HOLD_MAX_US=$batch2MedianActiveHoldMaxUs"
Write-Host "P3C_SPLIT2_ACTIVE_HOLD_MAX_US=$medianActiveHoldMax"
Write-Host ("P3C_SPLIT2_ACTIVE_HOLD_MAX_DELTA_VS_BASELINE_PCT={0:F2}" -f $holdMaxGainPct)
Write-Host "P3C_INTERPRETATION=$p3cInterpretation"
...
Write-Host ""
Write-Host "A14_P3C_SINGLE_VARIABLE=SECOND_UDP_RX_ATTEMPT_WITH_SPI_REACQUIRE"
Write-Host "A14_P3C_PRODUCT_SOURCE_MUTATION=NO"
Write-Host "A14_P3C_DIAGNOSTIC_COPY_ONLY=YES"
Write-Host "A14_P3C_UDP_RX_SPLIT2_REACQUIRE_AB=PASS"
Write-Host "NEXT_ACTION=RETURN_OUTPUT_TO_CHAT_FOR_PACING_VS_BATCHING_DECISION"
