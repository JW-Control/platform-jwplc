...
Write-Host "============================================================"
Write-Host " A14 P3I BLOCK - SINGLE-CS BURST VS FUSED @ 1016 B / 8x2KB / 26 MHz"
Write-Host "============================================================"
...
Write-Host "SINGLE_VARIABLE=UDP_RX_SINGLE_CS_BURST"
...
$burstPatchPath = Join-Path $PSScriptRoot "a14_p3i_udp_rx_single_cs_burst_patch.py"
...
$tempRoot = Join-Path $env:TEMP ("jwplc_a14_p3i_udp_rx_burst_ab_{0}" -f $timestamp)
...
$preflightBurstLog = Join-Path $preflightRoot "burst_patch.log"
$preflightBurstArgs = @(
    $burstPatchPath,
    "--ethernet-root", $preflightEthernetRoot
)

$preflightBurstExit = Invoke-NativeToLog -FilePath $pythonExe -Arguments $preflightBurstArgs -LogPath $preflightBurstLog
Write-Host "PREFLIGHT_BURST_PATCH_EXIT=$preflightBurstExit"

if ($preflightBurstExit -ne 0) {
    Get-Content -LiteralPath $preflightBurstLog -Tail 200 | ForEach-Object { Write-Host $_ }
    throw "P3I_PREFLIGHT_BURST_PATCH_FAILED"
}

Get-Content -LiteralPath $preflightBurstLog | ForEach-Object { Write-Host $_ }
Write-Host "P3I_PATCH_COMPOSITION_PREFLIGHT=PASS"
...
foreach ($variant in $variants) {
    ...
    if ($variant -eq "INT_BURST") {
        $burstLog = Join-Path $variantRoot "burst_patch.log"
        $burstArgs = @(
            $burstPatchPath,
            "--ethernet-root", $diagEthernetRoot
        )

        $burstExit = Invoke-NativeToLog -FilePath $pythonExe -Arguments $burstArgs -LogPath $burstLog
        Write-Host "BURST_PATCH_EXIT=$burstExit"

        if ($burstExit -ne 0) {
            Get-Content -LiteralPath $burstLog -Tail 180 | ForEach-Object { Write-Host $_ }
            throw "P3I_BURST_PATCH_FAILED"
        }

        Get-Content -LiteralPath $burstLog | ForEach-Object { Write-Host $_ }
    }
    else {
        Write-Host "BURST_PATCH=NOT_APPLIED"
    }
    ...
    foreach ($payload in $payloads) {
        ...
        for ($run = 1; $run -le $Runs; ++$run) {
            ...
            Write-Host "P3I_RUN VARIANT=$variant PAYLOAD=$payload RUN=$run RUNNER_EXIT=$runExit LOG=$runLog"
            ...
            Write-Host (
                "P3I_RESULT VARIANT={0} PAYLOAD={1} RUN={2} DUT_MBPS={3:F6} PACKETS={4} SERVICE_HOLDS={5} ACTIVE_HOLDS={6} EMPTY_HOLDS={7} EMPTY_HOLD_RATIO_PCT={8:F2} PACKETS_PER_ACTIVE_HOLD_X1000={9} ACTIVE_HOLD_AVG_US={10} ACTIVE_HOLD_MAX_US={11}" -f
                $variant,
                $payload,
                $run,
                $dutMbps,
        ...
        Write-Host (
            "P3I_MEDIAN VARIANT={0} PAYLOAD={1} DUT_MBPS={2:F6} SERVICE_HOLDS={3} EMPTY_HOLD_RATIO_PCT={4:F2} PACKETS_PER_ACTIVE_HOLD_X1000={5} ACTIVE_HOLD_AVG_US={6} ACTIVE_HOLD_MAX_US={7} INT_SKIPS={8} INT_WAKES={9} SPI_READS_PER_PACKET_X1000={10} TOTAL_W5500_READ_CALLS={11}" -f
            $variant,
            $payload,
            $medianDutMbps,
            $medianServiceHolds,
...
Write-Host ""
Write-Host "============================================================"
Write-Host " P3I COMPARATIVE SUMMARY"
Write-Host "============================================================"

foreach ($payload in $payloads) {
    $fused = $results["INT_FUSED|$payload"]
    $burst = $results["INT_BURST|$payload"]

    $throughputDeltaPct =
        (($burst.DutMbps / $fused.DutMbps) - 1.0) * 100.0

    $spiReadsReductionPct =
        (1.0 - (
            $burst.SpiReadsPerPacketX1000 /
            [double]$fused.SpiReadsPerPacketX1000
        )) * 100.0

    $totalReadReductionPct =
        (1.0 - (
            $burst.TotalReadCalls /
            [double]$fused.TotalReadCalls
        )) * 100.0

    Write-Host (
        "P3I_SUMMARY PAYLOAD={0} FUSED_MBPS={1:F6} BURST_MBPS={2:F6} THROUGHPUT_DELTA_PCT={3:F2} FUSED_SPI_READS_PER_PACKET_X1000={4} BURST_SPI_READS_PER_PACKET_X1000={5} SPI_READS_REDUCTION_PCT={6:F2} FUSED_TOTAL_READ_CALLS={7} BURST_TOTAL_READ_CALLS={8} TOTAL_READ_CALL_REDUCTION_PCT={9:F2} FUSED_HOLD_AVG_US={10} BURST_HOLD_AVG_US={11} FUSED_HOLD_MAX_US={12} BURST_HOLD_MAX_US={13}" -f
        $payload,
        $fused.DutMbps,
        $burst.DutMbps,
        $throughputDeltaPct,
        ...
        $burst.ActiveHoldAvgUs,
        $fused.ActiveHoldMaxUs,
        $burst.ActiveHoldMaxUs
    )
}

$fused1016 = $results["INT_FUSED|1016"]
$burst1016 = $results["INT_BURST|1016"]

$gain1016 =
    (($burst1016.DutMbps / $fused1016.DutMbps) - 1.0) * 100.0

$readsReduction1016 =
    (1.0 - (
        $burst1016.SpiReadsPerPacketX1000 /
        [double]$fused1016.SpiReadsPerPacketX1000
    )) * 100.0

$interpretation = "BURST_NO_CLEAR_BENEFIT"

if ($gain1016 -ge 1.0) {
    $interpretation = "BURST_THROUGHPUT_GAIN"
}
elseif (
    $burst1016.DutMbps -ge ($fused1016.DutMbps * 0.995) -and
    $readsReduction1016 -ge 10.0
) {
    $interpretation = "BURST_SPI_EFFICIENCY_GAIN"
}

Write-Host ""
Write-Host ("P3I_1016_THROUGHPUT_DELTA_PCT={0:F2}" -f $gain1016)
Write-Host ("P3I_1016_SPI_READ_REDUCTION_PCT={0:F2}" -f $readsReduction1016)
...
Write-Host ""
Write-Host "A14_P3I_SPI_HZ=26000000"
Write-Host "A14_P3I_SOCKET_TOPOLOGY=8x2KB_UNCHANGED"
Write-Host "A14_P3I_ALGORITHM=UDP_RX_BATCH2_INT_AND_FUSED_FROZEN"
Write-Host "A14_P3I_ETH_INT_PIN=GPIO15"
Write-Host "A14_P3I_ISR_SPI_ACCESS=NO"
Write-Host "A14_P3I_PRODUCT_SOURCE_MUTATION=NO"
Write-Host "A14_P3I_DIAGNOSTIC_COPY_ONLY=YES"
Write-Host "A14_P3I_PER_RUN_SERIAL_RESET_GUARD=YES"
Write-Host "A14_P3I_BLOCK_SINGLE_CS_BURST_AB=PASS"
Write-Host "NEXT_ACTION=P3I_REPEATABILITY_AGGREGATION"
