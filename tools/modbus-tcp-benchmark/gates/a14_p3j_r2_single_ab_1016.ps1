...
foreach ($payload in $payloads) {
    ...
    Write-Host (
        "P3J_SUMMARY PAYLOAD={0} FUSED_MBPS={1:F6} COMMIT2_R1_MBPS={2:F6} THROUGHPUT_DELTA_PCT={3:F2} FUSED_SPI_READS_PER_PACKET_X1000={4} COMMIT2_R1_SPI_READS_PER_PACKET_X1000={5} SPI_READS_REDUCTION_PCT={6:F2} FUSED_TOTAL_READ_CALLS={7} COMMIT2_R1_TOTAL_READ_CALLS={8} TOTAL_READ_CALL_REDUCTION_PCT={9:F2} FUSED_HOLD_AVG_US={10} COMMIT2_R1_HOLD_AVG_US={11} FUSED_HOLD_MAX_US={12} COMMIT2_R1_HOLD_MAX_US={13}" -f
        $payload,
        $fused.DutMbps,
        $commit2.DutMbps,
        $throughputDeltaPct,
