...
$payloads = @(1016)
$variants = @("INT_COMMIT2_R1", "INT_FUSED")
...
Write-Host "============================================================"
Write-Host " A14 P3J-R2 BLOCK - QUIESCENT LIFECYCLE + POST-COMMIT REARM VS FUSED @ 1016 B / 8x2KB / 26 MHz"
Write-Host "============================================================"
...
Write-Host "SINGLE_VARIABLE=UDP_RX_COALESCED_COMMIT_WITH_SHARED_QUIESCENT_LIFECYCLE"
...
$bridgePath = Join-Path $PSScriptRoot "a14_p3j_r2_quiescent_udp_rx_bridge.py"
$resolverPath = Join-Path $PSScriptRoot "a14_nb3_resolve_dut_ip.py"

$timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
$tempRoot = Join-Path $env:TEMP ("jwplc_a14_p3j_r2_udp_rx_commit_ab_{0}" -f $timestamp)
...
foreach ($variant in $variants) {
    ...
    foreach ($payload in $payloads) {
        ...
        for ($run = 1; $run -le $Runs; ++$run) {
            ...
            Write-Host "P3J_R2_RUN VARIANT=$variant PAYLOAD=$payload RUN=$run RUNNER_EXIT=$runExit LOG=$runLog"
            ...
            if ((Get-LogValue -Text $text -Key "P3J_R2_FINAL_SNAPSHOT_PRESENT") -ne "YES") {
                throw "P3G_SNAPSHOT_MISSING_$variant" + "_P$payload" + "_R$run"
            }

            if ((Get-LogValue -Text $text -Key "P3J_R2_ARM_RESET_PASS") -ne "YES") {
                throw "P3J_R2_ARM_RESET_GUARD_FAILED_$variant" + "_P$payload" + "_R$run"
            }

            if ((Get-LogInt64 -Text $text -Key "P3J_R2_ARM_RESET_RX_BYTES") -ne 0) {
                throw "P3J_R2_ARM_RESET_RX_BYTES_NONZERO_$variant" + "_P$payload" + "_R$run"
            }

            if ((Get-LogInt64 -Text $text -Key "P3J_R2_ARM_RESET_RX_OPERATIONS") -ne 0) {
                throw "P3J_R2_ARM_RESET_RX_OPERATIONS_NONZERO_$variant" + "_P$payload" + "_R$run"
            }

            if ((Get-LogValue -Text $text -Key "P3J_R2_QUIESCENCE_PASS") -ne "YES") {
                throw "P3J_R2_QUIESCENCE_FAILED_$variant" + "_P$payload" + "_R$run"
            }

            if ((Get-LogValue -Text $text -Key "P3J_R2_FINAL_ETH_READY") -ne "YES") {
                throw "P3G_ETH_NOT_READY_$variant" + "_P$payload" + "_R$run"
            }

            if ((Get-LogValue -Text $text -Key "P3J_R2_FINAL_ETH_LINK") -ne "UP") {
                throw "P3G_LINK_DOWN_$variant" + "_P$payload" + "_R$run"
            }

            if ((Get-LogValue -Text $text -Key "P3J_R2_FINAL_IP") -ne $dutIp) {
                throw "P3G_IP_MISMATCH_$variant" + "_P$payload" + "_R$run"
            }

            $transportErrors = Get-LogInt64 -Text $text -Key "P3J_R2_FINAL_TRANSPORT_ERRORS"
            $udpSpiLockErrors = Get-LogInt64 -Text $text -Key "P3J_R2_FINAL_UDP_SPI_LOCK_ERRORS"
            $packets = Get-LogInt64 -Text $text -Key "P3J_R2_FINAL_UDP_RX_PACKETS"
            $rxOperations = Get-LogInt64 -Text $text -Key "P3J_R2_FINAL_RX_OPERATIONS"
            $activeHolds = Get-LogInt64 -Text $text -Key "P3J_R2_FINAL_UDP_RX_ACTIVE_HOLD_COUNT"
            $serviceHolds = Get-LogInt64 -Text $text -Key "P3J_R2_FINAL_UDP_RX_SERVICE_HOLD_COUNT"
            $emptyHolds = Get-LogInt64 -Text $text -Key "P3J_R2_FINAL_UDP_RX_EMPTY_HOLD_COUNT"
            ...
            if ((Get-LogValue -Text $text -Key "P3J_R2_FINAL_ETH_INT_CONFIGURED") -ne "YES") {
                throw "P3H_INT_NOT_CONFIGURED_$variant" + "_P$payload" + "_R$run"
            }

            $intPin = Get-LogInt64 -Text $text -Key "P3J_R2_FINAL_ETH_INT_PIN"
            $intSocket = Get-LogInt64 -Text $text -Key "P3J_R2_FINAL_ETH_INT_UDP_SOCKET"
            $skipCount = Get-LogInt64 -Text $text -Key "P3J_R2_FINAL_UDP_RX_INT_SKIP_COUNT"
            $wakeCount = Get-LogInt64 -Text $text -Key "P3J_R2_FINAL_UDP_RX_INT_WAKE_COUNT"
            ...
            if ($dutMbps -lt 8.0) {
                throw "P3J_R1_THROUGHPUT_SANITY_FLOOR_$variant" + "_P$payload" + "_R$run=$dutMbps"
            }

            $activeHoldAvg = Get-LogInt64 -Text $text -Key "P3J_R2_FINAL_UDP_RX_ACTIVE_HOLD_US_AVG"
            $activeHoldMax = Get-LogInt64 -Text $text -Key "P3J_R2_FINAL_UDP_RX_ACTIVE_HOLD_US_MAX"
            $spiReadsPerPacketX1000 = Get-LogInt64 -Text $text -Key "P3J_R2_FINAL_UDP_RX_SPI_READS_PER_PACKET_X1000"
            $totalReadCalls = Get-LogInt64 -Text $text -Key "P3J_R2_FINAL_W5100_DIAG_READ_CALLS_TOTAL"
            ...
            Write-Host (
                "P3J_R2_RESULT VARIANT={0} PAYLOAD={1} RUN={2} DUT_MBPS={3:F6} PACKETS={4} SERVICE_HOLDS={5} ACTIVE_HOLDS={6} EMPTY_HOLDS={7} EMPTY_HOLD_RATIO_PCT={8:F2} PACKETS_PER_ACTIVE_HOLD_X1000={9} ACTIVE_HOLD_AVG_US={10} ACTIVE_HOLD_MAX_US={11}" -f
                $variant,
                $payload,
                $run,
                $dutMbps,
        ...
        Write-Host (
            "P3J_R2_MEDIAN VARIANT={0} PAYLOAD={1} DUT_MBPS={2:F6} SERVICE_HOLDS={3} EMPTY_HOLD_RATIO_PCT={4:F2} PACKETS_PER_ACTIVE_HOLD_X1000={5} ACTIVE_HOLD_AVG_US={6} ACTIVE_HOLD_MAX_US={7} INT_SKIPS={8} INT_WAKES={9} SPI_READS_PER_PACKET_X1000={10} TOTAL_W5500_READ_CALLS={11}" -f
            $variant,
            $payload,
            $medianDutMbps,
            $medianServiceHolds,
...
Write-Host ""
Write-Host "============================================================"
Write-Host " P3J-R2 COMPARATIVE SUMMARY"
Write-Host "============================================================"

foreach ($payload in $payloads) {
    $fused = $results["INT_FUSED|$payload"]
    $commit2 = $results["INT_COMMIT2_R1|$payload"]

    $throughputDeltaPct =
        (($commit2.DutMbps / $fused.DutMbps) - 1.0) * 100.0

    $spiReadsReductionPct =
        (1.0 - (
            $commit2.SpiReadsPerPacketX1000 /
            [double]$fused.SpiReadsPerPacketX1000
        )) * 100.0

    $totalReadReductionPct =
        (1.0 - (
            $commit2.TotalReadCalls /
            [double]$fused.TotalReadCalls
        )) * 100.0

    Write-Host (
        "P3J_R2_SUMMARY PAYLOAD={0} FUSED_MBPS={1:F6} COMMIT2_R1_MBPS={2:F6} THROUGHPUT_DELTA_PCT={3:F2} FUSED_SPI_READS_PER_PACKET_X1000={4} COMMIT2_R1_SPI_READS_PER_PACKET_X1000={5} SPI_READS_REDUCTION_PCT={6:F2} FUSED_TOTAL_READ_CALLS={7} COMMIT2_R1_TOTAL_READ_CALLS={8} TOTAL_READ_CALL_REDUCTION_PCT={9:F2} FUSED_HOLD_AVG_US={10} COMMIT2_R1_HOLD_AVG_US={11} FUSED_HOLD_MAX_US={12} COMMIT2_R1_HOLD_MAX_US={13}" -f
        $payload,
        $fused.DutMbps,
        $commit2.DutMbps,
        $throughputDeltaPct,
        ...
        $commit2.ActiveHoldAvgUs,
        $fused.ActiveHoldMaxUs,
        $commit2.ActiveHoldMaxUs
    )
}

$fused1016 = $results["INT_FUSED|1016"]
$commit21016 = $results["INT_COMMIT2_R1|1016"]

$gain1016 =
    (($commit21016.DutMbps / $fused1016.DutMbps) - 1.0) * 100.0

$readsReduction1016 =
    (1.0 - (
        $commit21016.SpiReadsPerPacketX1000 /
        [double]$fused1016.SpiReadsPerPacketX1000
    )) * 100.0

$interpretation = "COMMIT2_R1_NO_CLEAR_BENEFIT"

if ($gain1016 -ge 1.0) {
    $interpretation = "COMMIT2_R1_THROUGHPUT_GAIN"
}
elseif (
    $commit21016.DutMbps -ge ($fused1016.DutMbps * 0.995) -and
    $readsReduction1016 -ge 8.0
) {
    $interpretation = "COMMIT2_R1_SPI_EFFICIENCY_GAIN"
}

Write-Host ""
Write-Host ("P3J_R2_1016_THROUGHPUT_DELTA_PCT={0:F2}" -f $gain1016)
Write-Host ("P3J_R2_1016_SPI_READ_REDUCTION_PCT={0:F2}" -f $readsReduction1016)
Write-Host "P3J_R2_INTERPRETATION=$interpretation"
...
if ($finalHz -ne 26000000) { throw "P3J_R2_FINAL_FREQ_CHANGED" }
if ($finalDirty.Count -ne 0) { throw "P3G_PRODUCT_TREE_DIRTY" }
if ($finalStaged.Count -ne 0) { throw "P3G_PRODUCT_INDEX_DIRTY" }
if ($finalRawHash -ne $expectedRawHash) { throw "P3J_R2_FINAL_RAW_HASH_CHANGED" }
if ($finalW5100CppHash -ne $expectedW5100CppHash) { throw "P3J_R2_FINAL_W5100_CPP_HASH_CHANGED" }
if ($finalW5100HHash -ne $expectedW5100HHash) { throw "P3J_R2_FINAL_W5100_H_HASH_CHANGED" }

Write-Host ""
Write-Host "A14_P3J_R2_SPI_HZ=26000000"
Write-Host "A14_P3J_R2_SOCKET_TOPOLOGY=8x2KB_UNCHANGED"
Write-Host "A14_P3J_R2_ALGORITHM=UDP_RX_BATCH2_INT_AND_FUSED_FROZEN"
Write-Host "A14_P3J_R2_ETH_INT_PIN=GPIO15"
Write-Host "A14_P3J_R2_ISR_SPI_ACCESS=NO"
Write-Host "A14_P3J_R2_PRODUCT_SOURCE_MUTATION=NO"
Write-Host "A14_P3J_R2_DIAGNOSTIC_COPY_ONLY=YES"
Write-Host "A14_P3J_R2_PER_RUN_SERIAL_RESET_GUARD=YES"
Write-Host "A14_P3J_R2_SERIAL_IDLE_QUIESCENCE_BARRIER=YES"
Write-Host "A14_P3J_R2_BLOCK_QUIESCENT_POSTCOMMIT_REARM_AB=PASS"
Write-Host "NEXT_ACTION=P3J_R2_REPEATABILITY_AGGREGATION"
