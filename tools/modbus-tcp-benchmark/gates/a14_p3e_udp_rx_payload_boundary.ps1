...
$p3bReference1472Mbps = [double]11.335577
$udpRxBufferBytes = [int64]2048
$udpInternalHeaderBytes = [int64]8
$payloads = @(1472, 1017, 1016, 1000)
...
Write-Host "============================================================"
Write-Host " A14 P3E - UDP RX PAYLOAD BOUNDARY @ 8x2KB / 26 MHz"
Write-Host "============================================================"
...
Write-Host "P3B_REFERENCE_1472_DUT_MBPS=$p3bReference1472Mbps"
...
$tempRoot = Join-Path $env:TEMP ("jwplc_a14_p3e_udp_rx_payload_boundary_{0}" -f $timestamp)
...
Write-Host ""
Write-Host "=== P3E UDP RX PAYLOAD BOUNDARY RUNS ==="
...
foreach ($payload in $payloads) {
    ...
    for ($run = 1; $run -le $Runs; ++$run) {
        ...
        if ($packets -gt (2 * $activeHolds)) { throw "P3E_PACKETS_GT_BATCH2_CAPACITY_PAYLOAD_$payload" + "_RUN_$run" }
        ...
        Write-Host (
            "P3E_RUN PAYLOAD={0} RUN={1} DUT_MBPS={2:F6} PACKETS={3} ACTIVE_HOLDS={4} PACKETS_PER_ACTIVE_HOLD_X1000={5} SERVICE_HOLDS={6} EMPTY_HOLDS={7} EMPTY_HOLD_RATIO_PCT={8:F2} PARSE_AVG_US={9} READ_AVG_US={10} SPI_READS_PER_PACKET_X1000={11} ACTIVE_HOLD_AVG_US={12} ACTIVE_HOLD_MAX_US={13}" -f
            $payload,
            $run,
            $dutMbps,
            $packets,
    ...
    Write-Host (
        "P3E_MEDIAN PAYLOAD={0} RECORD_BYTES={1} COMPLETE_DATAGRAMS_PER_2KB={2} DUT_MBPS={3:F6} PACKETS_PER_ACTIVE_HOLD_X1000={4} EMPTY_HOLD_RATIO_PCT={5:F2} PARSE_AVG_US={6} READ_AVG_US={7} SPI_READS_PER_PACKET_X1000={8} ACTIVE_HOLD_AVG_US={9} ACTIVE_HOLD_MAX_US={10}" -f
        $payload,
        $recordBytes,
        $completeDatagramsPer2K,
        $medianDutMbps,
...
$boundaryPphDeltaX1000 =
    $r1016.PacketsPerActiveHoldX1000 -
    $r1017.PacketsPerActiveHoldX1000

$boundaryEmptyRatioDeltaPct =
    $r1016.EmptyHoldRatioPct -
    $r1017.EmptyHoldRatioPct

$boundaryThroughputDeltaPct =
    (($r1016.DutMbps / $r1017.DutMbps) - 1.0) * 100.0

$controlPphDeltaX1000 =
    $r1000.PacketsPerActiveHoldX1000 -
    $r1017.PacketsPerActiveHoldX1000

$repro1472DeltaPct =
    (($r1472.DutMbps / $p3bReference1472Mbps) - 1.0) * 100.0

$interpretation = "NO_CLEAR_BOUNDARY_EFFECT"
if (
    $r1016.PacketsPerActiveHoldX1000 -ge 1200 -and
    $boundaryPphDeltaX1000 -ge 100
) {
    $interpretation = "STRONG_2KB_BUFFER_BOUNDARY_EFFECT"
}
elseif ($boundaryPphDeltaX1000 -ge 50) {
    $interpretation = "BUFFER_BOUNDARY_EFFECT"
}
elseif ($controlPphDeltaX1000 -ge 50) {
    $interpretation = "CONTROL_EFFECT_WITHOUT_SHARP_1017_1016_BOUNDARY"
}

Write-Host ""
Write-Host "=== P3E BOUNDARY ANALYSIS ==="
Write-Host "P3E_1017_RECORD_BYTES=$($r1017.RecordBytes)"
Write-Host "P3E_1016_RECORD_BYTES=$($r1016.RecordBytes)"
Write-Host "P3E_1017_COMPLETE_DATAGRAMS_PER_2KB=$($r1017.CompleteDatagramsPer2K)"
Write-Host "P3E_1016_COMPLETE_DATAGRAMS_PER_2KB=$($r1016.CompleteDatagramsPer2K)"
Write-Host "P3E_1017_PACKETS_PER_ACTIVE_HOLD_X1000=$($r1017.PacketsPerActiveHoldX1000)"
Write-Host "P3E_1016_PACKETS_PER_ACTIVE_HOLD_X1000=$($r1016.PacketsPerActiveHoldX1000)"
Write-Host "P3E_1000_PACKETS_PER_ACTIVE_HOLD_X1000=$($r1000.PacketsPerActiveHoldX1000)"
Write-Host "P3E_BOUNDARY_PPH_DELTA_X1000=$boundaryPphDeltaX1000"
Write-Host ("P3E_1017_EMPTY_HOLD_RATIO_PCT={0:F2}" -f $r1017.EmptyHoldRatioPct)
Write-Host ("P3E_1016_EMPTY_HOLD_RATIO_PCT={0:F2}" -f $r1016.EmptyHoldRatioPct)
Write-Host ("P3E_BOUNDARY_EMPTY_HOLD_RATIO_DELTA_PCT={0:F2}" -f $boundaryEmptyRatioDeltaPct)
Write-Host ("P3E_1017_DUT_MBPS={0:F6}" -f $r1017.DutMbps)
Write-Host ("P3E_1016_DUT_MBPS={0:F6}" -f $r1016.DutMbps)
Write-Host ("P3E_BOUNDARY_THROUGHPUT_DELTA_PCT={0:F2}" -f $boundaryThroughputDeltaPct)
Write-Host ("P3E_1472_REPRODUCTION_DELTA_PCT={0:F2}" -f $repro1472DeltaPct)
Write-Host "P3E_INTERPRETATION=$interpretation"
...
Write-Host ""
Write-Host "A14_P3E_SOCKET_TOPOLOGY=8x2KB_UNCHANGED"
Write-Host "A14_P3E_ALGORITHM=UDP_RX_BATCH2_FROZEN"
Write-Host "A14_P3E_PRODUCT_SOURCE_MUTATION=NO"
Write-Host "A14_P3E_DIAGNOSTIC_COPY_ONLY=YES"
Write-Host "A14_P3E_UDP_RX_PAYLOAD_BOUNDARY=PASS"
Write-Host "NEXT_ACTION=RETURN_OUTPUT_TO_CHAT_FOR_2KB_BOUNDARY_DECISION"
