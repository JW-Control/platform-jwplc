...
if (-not (Test-Path -LiteralPath $ipResolverPath)) {
    throw "A14_NB3C_IP_RESOLVER_NOT_FOUND"
}
...
if ($ipResolverExit -ne 0) {
    ...
    throw "A14_NB3C_IP_RESOLVER_FAILED"
}
...
if ($ipMatches.Count -ne 1) {
    throw "A14_NB3C_EFFECTIVE_IP_COUNT_INVALID=$($ipMatches.Count)"
}
...
if (-not [string]::IsNullOrWhiteSpace($DutIp)) {
    $requestedMatch = ($DutIp -eq $effectiveDutIp)
    Write-Host "DUT_IP_REQUESTED_MATCH=$requestedMatch"

    if (-not $requestedMatch) {
        throw "A14_NB3C_REQUESTED_IP_MISMATCH requested=$DutIp actual=$effectiveDutIp"
    }
}
else {
