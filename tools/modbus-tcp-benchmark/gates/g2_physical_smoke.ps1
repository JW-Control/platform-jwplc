...
function Get-G2SnapshotInt64 {
    ...
    if (-not $Values.ContainsKey($Key)) {
        throw "G2_SNAPSHOT_KEY_MISSING=$Key"
    }
...
function Assert-G2SnapshotValue {
    param(
        [Parameter(Mandatory = $true)]
        [hashtable]$Values,
    ...
    Write-Host "SNAPSHOT_${Key}=$actual"

    if ($actual -ne $Expected) {
        throw "G2_SNAPSHOT_${Key}_MISMATCH"
    }
}

function Assert-G2SnapshotZero {
    param(
        [Parameter(Mandatory = $true)]
        [hashtable]$Values,
    ...
    $value = Get-G2SnapshotInt64 -Values $Values -Key $Key
    Write-Host "SNAPSHOT_${Key}=$value"

    if ($value -ne 0) {
        throw "G2_SNAPSHOT_${Key}_NONZERO=$value"
    }
}

function Get-G2SerialSnapshot {
    param(
        [Parameter(Mandatory = $true)]
        [string]$PortName,
    ...
    if (-not $raw.Contains("ETH14_RAW_SNAPSHOT=END")) {
        throw "G2_SERIAL_SNAPSHOT_TIMEOUT"
    }
...
function Assert-G2PostModeSnapshot {
    param(
        [Parameter(Mandatory = $true)]
        [hashtable]$Values,
    ...
    Assert-G2SnapshotValue -Values $Values -Key "RAW_SERVER_READY" -Expected "YES"
    Assert-G2SnapshotValue -Values $Values -Key "ETH_READY" -Expected "YES"
    Assert-G2SnapshotValue -Values $Values -Key "ETH_LINK" -Expected "UP"
    Assert-G2SnapshotValue -Values $Values -Key "IP" -Expected $ExpectedIp
    ...
    @(
        ...
        Assert-G2SnapshotZero -Values $Values -Key $_
    }

    $holdMaxUs = Get-G2SnapshotInt64 -Values $Values -Key "TCP_SPI_HOLD_MAX_US"
    Write-Host "SNAPSHOT_TCP_SPI_HOLD_MAX_US=$holdMaxUs"

    if ($holdMaxUs -gt $script:G2HoldBudgetUs) {
        throw "G2_TCP_SPI_HOLD_BUDGET_EXCEEDED=$holdMaxUs"
    }
...
$runnerPath = Get-G2Path $script:G2RawRunnerRelative
$librariesRoot = Get-G2Path "JWPLC/2.1.0/libraries"
$expectedEthernetLibrary = Join-Path $librariesRoot "JWPLC_Ethernet"

$timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
$tempRoot = Join-Path $env:TEMP ("jwplc_g2_{0}mhz_smoke_{1}" -f $MHz, $timestamp)
...
Write-Host ""
Write-Host "=== FOUR-MODE SMOKE ==="

foreach ($mode in $modeMap) {
    $runLog = Join-Path $tempRoot ("{0}.log" -f $mode.Key.ToLowerInvariant())
    $snapshotLog = Join-Path $tempRoot ("{0}_snapshot.log" -f $mode.Key.ToLowerInvariant())
    ...
    if ($runExit -ne 0) {
        Get-Content -LiteralPath $runLog -Tail 100 | ForEach-Object { Write-Host $_ }
    ...
    $snapshot = Get-G2SerialSnapshot -PortName $SerialPort
    [System.IO.File]::WriteAllText($snapshotLog, $snapshot.Raw)
    $holdMaxUs = Assert-G2PostModeSnapshot -Values $snapshot.Values -ExpectedIp $DutIp
    ...
    if ($mode.Key -eq "UDP_TX") {
        @(
            ...
            $sequenceValue = [int64]::Parse(
                (Get-G2LogValue -Text $runText -Key $_),
                [System.Globalization.CultureInfo]::InvariantCulture
            )
        ...
        $sequenceCount = [int64]::Parse(
            (Get-G2LogValue -Text $runText -Key "UDP_TX_SEQUENCE_TOTAL_COUNT"),
            [System.Globalization.CultureInfo]::InvariantCulture
        )
...
if ($visualEvents -ne 0) {
    $summaryLines.Add("G2_PHYSICAL_SMOKE=FAIL")
    $summaryLines | Set-Content -LiteralPath $summaryLog -Encoding UTF8
    Write-Host "SUMMARY_LOG=$summaryLog"
    Write-Host "G2_PHYSICAL_SMOKE=FAIL"
    exit 2
}

$summaryLines.Add("G2_PHYSICAL_SMOKE=PASS")
$summaryLines | Set-Content -LiteralPath $summaryLog -Encoding UTF8

Write-Host ""
Write-Host "============================================================"
Write-Host " G2 PHYSICAL SMOKE SUMMARY"
Write-Host "============================================================"
Write-Host "MHZ=$MHz"
Write-Host "COMPILE_EXIT=$compileExit"
Write-Host "UPLOAD_EXIT=$uploadExit"
...
Write-Host "SUMMARY_LOG=$summaryLog"
Write-Host "G2_PHYSICAL_SMOKE=PASS"