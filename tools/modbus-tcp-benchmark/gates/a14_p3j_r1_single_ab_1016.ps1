...
Write-Host "============================================================"
Write-Host " A14 P3J-R1 BLOCK - POST-COMMIT INT REARM VS FUSED @ 1016 B / 8x2KB / 26 MHz"
Write-Host "============================================================"
...
Write-Host "SINGLE_VARIABLE=UDP_RX_COALESCED_COMMIT_POSTCOMMIT_REARM"
...
$tempRoot = Join-Path $env:TEMP ("jwplc_a14_p3j_r1_udp_rx_commit_ab_{0}" -f $timestamp)
...
Write-Host "P3J_R1_PATCH_COMPOSITION_PREFLIGHT=PASS"
...
foreach ($variant in $variants) {
    ...
    foreach ($payload in $payloads) {
        ...
        for ($run = 1; $run -le $Runs; ++$run) {
            ...
            Write-Host "P3J_R1_RUN VARIANT=$variant PAYLOAD=$payload RUN=$run RUNNER_EXIT=$runExit LOG=$runLog"
            ...
            Write-Host (
                "P3J_R1_RESULT VARIANT={0} PAYLOAD={1} RUN={2} DUT_MBPS={3:F6} PACKETS={4} SERVICE_HOLDS={5} ACTIVE_HOLDS={6} EMPTY_HOLDS={7} EMPTY_HOLD_RATIO_PCT={8:F2} PACKETS_PER_ACTIVE_HOLD_X1000={9} ACTIVE_HOLD_AVG_US={10} ACTIVE_HOLD_MAX_US={11}" -f
                $variant,
                $payload,
                $run,
                $dutMbps,
        ...
        Write-Host (
            "P3J_R1_MEDIAN VARIANT={0} PAYLOAD={1} DUT_MBPS={2:F6} SERVICE_HOLDS={3} EMPTY_HOLD_RATIO_PCT={4:F2} PACKETS_PER_ACTIVE_HOLD_X1000={5} ACTIVE_HOLD_AVG_US={6} ACTIVE_HOLD_MAX_US={7} INT_SKIPS={8} INT_WAKES={9} SPI_READS_PER_PACKET_X1000={10} TOTAL_W5500_READ_CALLS={11}" -f
            $variant,
            $payload,
            $medianDutMbps,
            $medianServiceHolds,
...
Write-Host ""
Write-Host "============================================================"
Write-Host " P3J-R1 COMPARATIVE SUMMARY"
Write-Host "============================================================"
...
Write-Host ""
Write-Host ("P3J_R1_1016_THROUGHPUT_DELTA_PCT={0:F2}" -f $gain1016)
Write-Host ("P3J_R1_1016_SPI_READ_REDUCTION_PCT={0:F2}" -f $readsReduction1016)
Write-Host "P3J_R1_INTERPRETATION=$interpretation"
...
Write-Host ""
Write-Host "A14_P3J_R1_SPI_HZ=26000000"
Write-Host "A14_P3J_R1_SOCKET_TOPOLOGY=8x2KB_UNCHANGED"
Write-Host "A14_P3J_R1_ALGORITHM=UDP_RX_BATCH2_INT_AND_FUSED_FROZEN"
Write-Host "A14_P3J_R1_ETH_INT_PIN=GPIO15"
Write-Host "A14_P3J_R1_ISR_SPI_ACCESS=NO"
Write-Host "A14_P3J_R1_PRODUCT_SOURCE_MUTATION=NO"
Write-Host "A14_P3J_R1_DIAGNOSTIC_COPY_ONLY=YES"
Write-Host "A14_P3J_R1_PER_RUN_SERIAL_RESET_GUARD=YES"
Write-Host "A14_P3J_R1_BLOCK_POSTCOMMIT_REARM_AB=PASS"
Write-Host "NEXT_ACTION=P3J_R1_REPEATABILITY_AGGREGATION"
