...
Write-Host "============================================================"
Write-Host " A14 P5 FINAL COMBINED HARNESS"
Write-Host " RTU50 + TCP + FULL RUNTIME CURRENT CONTRACT"
Write-Host "============================================================"
...
Write-Host "P5_CONTRACT=FULL_RUNTIME_CURRENT_PLUS_RTU"
Write-Host "OBSOLETE_DATALOG_MARKERS_REQUIRED=NO"
...
$CombinedReady = Has-Marker -Text $DutText -Marker "COMBINED_RUNTIME_READY=YES"
$FullRuntime = Has-Marker -Text $DutText -Marker "FULL_RUNTIME_READY=YES"
$ServerReady = Has-Marker -Text $DutText -Marker "SERVER_READY=YES"
$SdReady = Has-Marker -Text $DutText -Marker "SD_READY=YES"

$SdAppendCycles = Get-IntValue -Text $DutText -Key "SD_APPEND_CYCLES"
$SdAppendFails = Get-IntValue -Text $DutText -Key "SD_APPEND_FAILS"
$SdVerifyCycles = Get-IntValue -Text $DutText -Key "SD_VERIFY_CYCLES"
$SdVerifyFails = Get-IntValue -Text $DutText -Key "SD_VERIFY_FAILS"

$SdActivityPass = (
    $SdReady -and
    $SdAppendCycles -gt 0 -and
    $SdAppendFails -eq 0 -and
    $SdVerifyCycles -gt 0 -and
    $SdVerifyFails -eq 0
)

$PeripheralZero = Has-Marker -Text $DutText -Marker "PERIPHERAL_FAILURE_COUNT=0"
$RtuReady = Has-Marker -Text $DutText -Marker "RTU_READY=YES"
$RtuSlave2 = Has-Marker -Text $DutText -Marker "RTU_SLAVE_ID=2"

$DutRx = Get-IntValue -Text $DutText -Key "RTU_RX_FRAMES"
$DutTx = Get-IntValue -Text $DutText -Key "RTU_TX_FRAMES"
$DutOk = Get-IntValue -Text $DutText -Key "RTU_REQUESTS_OK"
$DutCrc = Get-IntValue -Text $DutText -Key "RTU_CRC_ERRORS"
$DutEx = Get-IntValue -Text $DutText -Key "RTU_EXCEPTIONS_SENT"
$DutLastOk = Has-Marker -Text $DutText -Marker "RTU_LAST_ERROR=OK"
$ServiceGap = Get-IntValue -Text $DutText -Key "RTU_SERVICE_GAP_MAX_US"

# Tolerancia histórica exclusiva del desfase temporal entre snapshots.
$RtuSnapshotTailTolerance = 12

$DutRxDelta = [Math]::Abs([int64]$Success - [int64]$DutRx)
$DutTxDelta = [Math]::Abs([int64]$Success - [int64]$DutTx)
$DutOkDelta = [Math]::Abs([int64]$Success - [int64]$DutOk)
...
Write-Host ""
Write-Host "=== COM14 P5 COMBINED ASSERTIONS ==="
Write-Host "COMBINED_RUNTIME_READY=$CombinedReady"
Write-Host "FULL_RUNTIME_READY=$FullRuntime"
Write-Host "SERVER_READY=$ServerReady"
Write-Host "SD_READY=$SdReady"
Write-Host "SD_APPEND_CYCLES=$SdAppendCycles"
Write-Host "SD_APPEND_FAILS=$SdAppendFails"
Write-Host "SD_VERIFY_CYCLES=$SdVerifyCycles"
Write-Host "SD_VERIFY_FAILS=$SdVerifyFails"
Write-Host "SD_ACTIVITY_PASS=$SdActivityPass"
Write-Host "PERIPHERAL_FAILURE_COUNT_ZERO=$PeripheralZero"
Write-Host "RTU_READY=$RtuReady"
Write-Host "RTU_SLAVE_2=$RtuSlave2"
...
Write-Host ""
Write-Host "============================================================"
Write-Host " A14 P5 COMBINED SUMMARY"
Write-Host "============================================================"
...
if ($OverallPass) {
    Write-Host ""
    Write-Host "A14_P5_COMBINED_HARNESS=PASS_PHYSICAL"
}
else {
    Write-Host ""
    Write-Host "A14_P5_COMBINED_HARNESS=REVIEW"
    throw "A14_P5_COMBINED_HARNESS_NOT_CLOSED"
}