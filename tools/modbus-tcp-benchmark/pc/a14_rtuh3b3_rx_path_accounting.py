...
def main() -> int:
    ...
    print("=" * 78)
    print(" A14 RTU-H3B.3 - RX-PATH ACCOUNTING + BULK RX + TCP500")
    print("=" * 78)
    ...
    print(f"RTU_DIAG_FLOOR_HZ={RTU_DIAG_FLOOR_HZ:.0f}")
    ...
    try:
        ...
        if not profile_pass:
            raise RuntimeError("perfil fijo H3B2 invalido")
        ...
        rtu_floor_pass = rtu_hz >= RTU_DIAG_FLOOR_HZ
        ...
        if rtu_timeouts == 0:
            diagnosis = "NO_FAILURE_REPRODUCED"
        elif slave_discarded_bytes > 0:
            diagnosis = "SERVER_PARSER_DISCARD_OBSERVED"
        elif request_byte_gap > 0:
            diagnosis = "PRE_PARSER_RX_BYTE_LOSS_CANDIDATE"
        elif request_path_gap > 0:
            diagnosis = "FRAME_ACCOUNTING_GAP_WITH_FULL_RX_BYTES"
        else:
            diagnosis = "TIMEOUT_WITHOUT_REQUEST_PATH_GAP"

        print()
        print("=" * 78)
        print(" RTU-H3B.3 LONG-RUN SUMMARY")
        print("=" * 78)
        ...
        print(f"RTUH3B3_TCP_REQ_S={float(tcp['achieved_req_s']):.3f}")
        print(f"RTUH3B3_TCP_TARGET_PCT={float(tcp['target_pct']):.3f}")
        print(f"RTUH3B3_TCP_AVG_US={float(tcp['avg_us']):.1f}")
        print(f"RTUH3B3_TCP_P95_US={float(tcp['p95_us']):.1f}")
        print(f"RTUH3B3_TCP_P99_US={float(tcp['p99_us']):.1f}")
        print(f"RTUH3B3_TCP_MAX_US={float(tcp['max_us']):.1f}")
        print(f"RTUH3B3_TCP_BUCKET_MIN_REQ_S={bucket_min:.3f}")
        print(f"RTUH3B3_TCP_BUCKET_MAX_REQ_S={bucket_max:.3f}")
        print(f"RTUH3B3_TCP_FIRST_HALF_AVG_REQ_S={first_rate_avg:.3f}")
        print(f"RTUH3B3_TCP_SECOND_HALF_AVG_REQ_S={second_rate_avg:.3f}")
        print(f"RTUH3B3_TCP_HALF_RATE_DRIFT_PCT={rate_drift_pct:.3f}")
        print(f"RTUH3B3_TCP_FIRST_HALF_AVG_US={first_latency_avg:.1f}")
        print(f"RTUH3B3_TCP_SECOND_HALF_AVG_US={second_latency_avg:.1f}")
        print(f"RTUH3B3_TCP_HALF_LATENCY_DRIFT_PCT={latency_drift_pct:.3f}")
        print(f"RTUH3B3_TCP_FIRST_HALF_P95_AVG_US={first_p95_avg:.1f}")
        print(f"RTUH3B3_TCP_SECOND_HALF_P95_AVG_US={second_p95_avg:.1f}")

        print(f"RTUH3B3_RTU_HZ={rtu_hz:.3f}")
        print(f"RTUH3B3_RTU_STARTED={started}")
        print(f"RTUH3B3_RTU_COMPLETED={completed}")
        print(f"RTUH3B3_RTU_SUCCESS={success}")
        print(f"RTUH3B3_RTU_FAILED={failed}")
        print(f"RTUH3B3_RTU_TIMEOUTS={rtu_timeouts}")
        print(f"RTUH3B3_MASTER_TX={master_tx}")
        print(f"RTUH3B3_MASTER_RX={master_rx}")
        print(f"RTUH3B3_SLAVE_RX={slave_rx}")
        print(f"RTUH3B3_SLAVE_TX={slave_tx}")
        print(f"RTUH3B3_SLAVE_OK={slave_ok}")
        print(f"RTUH3B3_REQUEST_PATH_GAP={request_path_gap}")
        print(f"RTUH3B3_RESPONSE_PATH_GAP={response_path_gap}")
        print(f"RTUH3B3_MASTER_RX_BYTES={master_rx_bytes}")
        print(f"RTUH3B3_MASTER_TX_BYTES={master_tx_bytes}")
        print(f"RTUH3B3_SLAVE_RX_BYTES={slave_rx_bytes}")
        print(f"RTUH3B3_SLAVE_TX_BYTES={slave_tx_bytes}")
        print(f"RTUH3B3_EXPECTED_MASTER_TX_BYTES={expected_master_tx_bytes}")
        print(f"RTUH3B3_EXPECTED_SLAVE_TX_BYTES={expected_slave_tx_bytes}")
        print(f"RTUH3B3_REQUEST_BYTE_GAP={request_byte_gap}")
        print(f"RTUH3B3_RESPONSE_BYTE_GAP={response_byte_gap}")
        print(f"RTUH3B3_MASTER_DISCARDED_TAILS={master_discarded_tails}")
        print(f"RTUH3B3_MASTER_DISCARDED_BYTES={master_discarded_bytes}")
        print(f"RTUH3B3_SLAVE_DISCARDED_TAILS={slave_discarded_tails}")
        print(f"RTUH3B3_SLAVE_DISCARDED_BYTES={slave_discarded_bytes}")
        print(
            "RTUH3B3_MASTER_TX_BYTE_ACCOUNTING_PASS="
            f"{'YES' if master_tx_byte_accounting_pass else 'NO'}"
        )
        print(
            "RTUH3B3_SLAVE_TX_BYTE_ACCOUNTING_PASS="
            f"{'YES' if slave_tx_byte_accounting_pass else 'NO'}"
        )
        print(f"RTUH3B3_DIAGNOSIS={diagnosis}")
        print(f"RTUH3B3_MASTER_CRC={master_crc}")
        print(f"RTUH3B3_SLAVE_CRC={slave_crc}")

        print(f"RTUH3B3_TRANSACTION_MAX_US={txn_max_us}")
        print(f"RTUH3B3_TRANSACTIONS_OVER_5MS={over_5ms}")
        print(f"RTUH3B3_TRANSACTIONS_OVER_10MS={over_10ms}")
        print(f"RTUH3B3_TRANSACTIONS_OVER_20MS={over_20ms}")
        print(f"RTUH3B3_LAST_FAILURE_DURATION_US={last_failure_us}")
        print(f"RTUH3B3_MAX_FAILURE_DURATION_US={max_failure_us}")
        print(f"RTUH3B3_LAST_FAILURE_RESULT={last_failure_result}")
        print(f"RTUH3B3_RTU_SERVICE_GAP_MAX_US={service_gap_max_us}")
        print(f"RTUH3B3_LOOP_GAP_MAX_US={loop_gap_max_us}")

        print(f"RTUH3B3_SD_ACTIVE={'YES' if sd_active else 'NO'}")
        print(f"RTUH3B3_SD_ACCEPTED_BYTES={sd_accepted}")
        print(f"RTUH3B3_SD_COMMITTED_BYTES={sd_committed}")
        print(f"RTUH3B3_SD_PENDING_BYTES={sd_pending}")
        print(f"RTUH3B3_SD_FAILED_COMMITS={sd_failed}")
        print(f"RTUH3B3_PERIPHERAL_FAILURE_COUNT={peripheral_failures}")

        print(f"RTUH3B3_PROFILE_PASS={'YES' if final_profile_pass else 'NO'}")
        print(f"RTUH3B3_RTU_CLEAN={'YES' if rtu_clean else 'NO'}")
        print(f"RTUH3B3_RTU_FLOOR_PASS={'YES' if rtu_floor_pass else 'NO'}")
        print(f"RTUH3B3_TCP_CLEAN={'YES' if tcp_clean else 'NO'}")
        print(f"RTUH3B3_TCP_TARGET_PASS={'YES' if tcp_target_pass else 'NO'}")
        print(f"RTUH3B3_BUCKET_TARGET_PASS={'YES' if bucket_target_pass else 'NO'}")
        print(f"RTUH3B3_SD_CLEAN={'YES' if sd_clean else 'NO'}")
        print(f"RTUH3B3_RUNTIME_CLEAN={'YES' if runtime_clean else 'NO'}")

        if runtime_clean:
            print("A14_RTU_H3B3=PASS_DIAGNOSTIC_LONGRUN_CLEAN")
        else:
            print("A14_RTU_H3B3=PASS_DIAGNOSTIC_CAPTURE_WITH_FAILURE")
