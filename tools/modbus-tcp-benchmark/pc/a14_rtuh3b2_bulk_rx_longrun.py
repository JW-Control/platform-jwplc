...
def main() -> int:
    ...
    print("=" * 78)
    print(" A14 RTU-H3B.2 - BULK RX + TCP500 LONG-RUN")
    print("=" * 78)
    ...
    print(f"RTU_LONGRUN_FLOOR_HZ={RTU_LONGRUN_FLOOR_HZ:.0f}")
    ...
    try:
        ...
        rtu_floor_pass = rtu_hz >= RTU_LONGRUN_FLOOR_HZ
        ...
        print()
        print("=" * 78)
        print(" RTU-H3B.2 LONG-RUN SUMMARY")
        print("=" * 78)
        ...
        print(f"RTUH3B2_TCP_REQ_S={float(tcp['achieved_req_s']):.3f}")
        print(f"RTUH3B2_TCP_TARGET_PCT={float(tcp['target_pct']):.3f}")
        print(f"RTUH3B2_TCP_AVG_US={float(tcp['avg_us']):.1f}")
        print(f"RTUH3B2_TCP_P95_US={float(tcp['p95_us']):.1f}")
        print(f"RTUH3B2_TCP_P99_US={float(tcp['p99_us']):.1f}")
        print(f"RTUH3B2_TCP_MAX_US={float(tcp['max_us']):.1f}")
        print(f"RTUH3B2_TCP_BUCKET_MIN_REQ_S={bucket_min:.3f}")
        print(f"RTUH3B2_TCP_BUCKET_MAX_REQ_S={bucket_max:.3f}")
        print(f"RTUH3B2_TCP_FIRST_HALF_AVG_REQ_S={first_rate_avg:.3f}")
        print(f"RTUH3B2_TCP_SECOND_HALF_AVG_REQ_S={second_rate_avg:.3f}")
        print(f"RTUH3B2_TCP_HALF_RATE_DRIFT_PCT={rate_drift_pct:.3f}")
        print(f"RTUH3B2_TCP_FIRST_HALF_AVG_US={first_latency_avg:.1f}")
        print(f"RTUH3B2_TCP_SECOND_HALF_AVG_US={second_latency_avg:.1f}")
        print(f"RTUH3B2_TCP_HALF_LATENCY_DRIFT_PCT={latency_drift_pct:.3f}")
        print(f"RTUH3B2_TCP_FIRST_HALF_P95_AVG_US={first_p95_avg:.1f}")
        print(f"RTUH3B2_TCP_SECOND_HALF_P95_AVG_US={second_p95_avg:.1f}")

        print(f"RTUH3B2_RTU_HZ={rtu_hz:.3f}")
        print(f"RTUH3B2_RTU_STARTED={started}")
        print(f"RTUH3B2_RTU_COMPLETED={completed}")
        print(f"RTUH3B2_RTU_SUCCESS={success}")
        print(f"RTUH3B2_RTU_FAILED={failed}")
        print(f"RTUH3B2_RTU_TIMEOUTS={rtu_timeouts}")
        print(f"RTUH3B2_MASTER_TX={master_tx}")
        print(f"RTUH3B2_MASTER_RX={master_rx}")
        print(f"RTUH3B2_SLAVE_RX={slave_rx}")
        print(f"RTUH3B2_SLAVE_TX={slave_tx}")
        print(f"RTUH3B2_SLAVE_OK={slave_ok}")
        print(f"RTUH3B2_REQUEST_PATH_GAP={request_path_gap}")
        print(f"RTUH3B2_RESPONSE_PATH_GAP={response_path_gap}")
        print(f"RTUH3B2_MASTER_CRC={master_crc}")
        print(f"RTUH3B2_SLAVE_CRC={slave_crc}")

        print(f"RTUH3B2_TRANSACTION_MAX_US={txn_max_us}")
        print(f"RTUH3B2_TRANSACTIONS_OVER_5MS={over_5ms}")
        print(f"RTUH3B2_TRANSACTIONS_OVER_10MS={over_10ms}")
        print(f"RTUH3B2_TRANSACTIONS_OVER_20MS={over_20ms}")
        print(f"RTUH3B2_LAST_FAILURE_DURATION_US={last_failure_us}")
        print(f"RTUH3B2_MAX_FAILURE_DURATION_US={max_failure_us}")
        print(f"RTUH3B2_LAST_FAILURE_RESULT={last_failure_result}")
        print(f"RTUH3B2_RTU_SERVICE_GAP_MAX_US={service_gap_max_us}")
        print(f"RTUH3B2_LOOP_GAP_MAX_US={loop_gap_max_us}")

        print(f"RTUH3B2_SD_ACTIVE={'YES' if sd_active else 'NO'}")
        print(f"RTUH3B2_SD_ACCEPTED_BYTES={sd_accepted}")
        print(f"RTUH3B2_SD_COMMITTED_BYTES={sd_committed}")
        print(f"RTUH3B2_SD_PENDING_BYTES={sd_pending}")
        print(f"RTUH3B2_SD_FAILED_COMMITS={sd_failed}")
        print(f"RTUH3B2_PERIPHERAL_FAILURE_COUNT={peripheral_failures}")

        print(f"RTUH3B2_PROFILE_PASS={'YES' if final_profile_pass else 'NO'}")
        print(f"RTUH3B2_RTU_CLEAN={'YES' if rtu_clean else 'NO'}")
        print(f"RTUH3B2_RTU_FLOOR_PASS={'YES' if rtu_floor_pass else 'NO'}")
        print(f"RTUH3B2_TCP_CLEAN={'YES' if tcp_clean else 'NO'}")
        print(f"RTUH3B2_TCP_TARGET_PASS={'YES' if tcp_target_pass else 'NO'}")
        print(f"RTUH3B2_BUCKET_TARGET_PASS={'YES' if bucket_target_pass else 'NO'}")
        print(f"RTUH3B2_SD_CLEAN={'YES' if sd_clean else 'NO'}")
        print(f"RTUH3B2_RUNTIME_CLEAN={'YES' if runtime_clean else 'NO'}")

        if not runtime_clean:
            print("A14_RTU_H3B2=REVIEW_LONGRUN_FAILURE")
            return 2

        print("A14_RTU_H3B2=PASS_BULK_LONGRUN_600S")
        return 0
