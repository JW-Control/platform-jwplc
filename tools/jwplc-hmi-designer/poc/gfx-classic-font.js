(() => {
  ...
  // Subconjunto ASCII imprimible (32..126) copiado del glcdfont.c incluido
  // en el package JWPLC/Adafruit_GFX_Library. Cada glifo ocupa 5 columnas;
  // drawChar() de Adafruit GFX usa una celda lógica de 6x8 incluyendo spacing.
  const FIRST = 32;
  const LAST = 126;
  const BYTES_PER_GLYPH = 5;
  ...
  function glyphFor(code) {
    if (code < FIRST || code > LAST) {
      code = 63; // '?' para el PoC ASCII V1.
    }

    const offset = (code - FIRST) * BYTES_PER_GLYPH;
    return DATA.subarray(offset, offset + BYTES_PER_GLYPH);
  }

  window.JWPLCGfxClassicFont = Object.freeze({
    first: FIRST,
    last: LAST,
    bytesPerGlyph: BYTES_PER_GLYPH,
    cellWidth: 6,
    cellHeight: 8,
    glyphFor
  });
