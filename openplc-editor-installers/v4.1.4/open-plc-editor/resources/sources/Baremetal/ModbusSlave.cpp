...
void mbconfig_ethernet_iface(uint8_t *mac, uint8_t *ip, uint8_t *dns, uint8_t *gateway, uint8_t *subnet)
{
    ...
    // Ethernet.begin() directo: la libreria JWPLC_Ethernet inicializa CS,
    // SPI, timeouts, DHCP/IP estatica, link y estado del W5500.
    ...
    if (ip == NULL)
    {
        ...
        ethOk = JWPLC_Ethernet.begin(mac);
    }
    else
    {
#if defined(JWPLC_MBTCP_DEBUG)
    ...
    // En algunas condiciones el W5500 puede obtener IP por DHCP aunque
    // JWPLC_Ethernet.begin() devuelva false por una validacion posterior
    // de link/estado. Para pruebas OpenPLC, si hay IP valida se levanta
    // el servidor TCP igualmente.
    if (ethOk || hasIp)
    {
        mb_server.begin();
...
void mbtask()
{
    ...
    // Cede tiempo al scheduler ESP32 para evitar que el polling TCP afecte
    // tareas JWPLC de TFT, E/S, RTC, Ethernet tick, etc.
    delay(1);
#else
    handle_tcp();
#endif
    ...
    for (int i = 0; i < MAX_SRV_CLIENTS; i++)
    {
...

        {
            ...
            while (mb_serverClients[i].available())
            {
                ...
                j++;
                if (j == MBAP_SIZE)
                    break; // MBAP has 6 bytes (we use UnitID as SlaveID)
            }

            mb_frame_len = mb_mbap[4] << 8 | mb_mbap[5];

            if (mb_mbap[2] != 0 || mb_mbap[3] != 0)
                return; // Not a MODBUSIP packet
            if (mb_frame_len < 6 || mb_frame_len > MAX_MB_FRAME)
                return; // Packet is too small or too big
    ...
    if (client)
    {
        if (client.connected())
        {
            ...
            while (client.available())
            {
                ...
                i++;
                if (i == mb_frame_len || i == MAX_MB_FRAME)
                    break;
            }
...
void handle_serial()
{
    ...
    if (mb_frame_len + 2 > MAX_MB_FRAME)
        exceptionResponse(mb_frame[1], MB_EX_SLAVE_FAILURE);
...
void readCoils(uint16_t startreg, uint16_t numregs)
{
    ...
    if (numregs % 8)
        mb_frame_len++; // Add 1 to the message length for the partial byte.
    if (mb_frame_len > MAX_MB_FRAME)
    {
        // Response message is too big for this device
