...
static const int16_t GRAVITY_PX_S2 = 300;
static const int16_t FLAP_VEL_PX_S = -100;
...
// =====================================================
// Audio no bloqueante - Flappy Bird
// =====================================================

static const bool ENABLE_BUZZER = true;
static const bool ENABLE_MUSIC = true;
static const bool ENABLE_SFX = true;

// Usa el mismo pin que usaste en Tetris.
...
// 64  = alto
// 128 = fuerte
static uint8_t buzzerVolumeMusic = 16;
static uint8_t buzzerVolumeSfx = 48;
...
// =====================================================
// Música estilo plataforma retro / tuberías
// Original, inspirada en chiptune clásico
// =====================================================

static const MusicNote FLAPPY_THEME[] = {
  {NOTE_E5, 90},  {NOTE_REST, 35}, {NOTE_E5, 90},  {NOTE_REST, 90},
  {NOTE_E5, 90},  {NOTE_REST, 90}, {NOTE_C5, 90},  {NOTE_E5, 90},
  {NOTE_G5, 180}, {NOTE_REST, 180}, {NOTE_G4, 180}, {NOTE_REST, 180},

  {NOTE_C5, 130}, {NOTE_REST, 70}, {NOTE_G4, 130}, {NOTE_REST, 70},
  {NOTE_E4, 130}, {NOTE_REST, 70},
  {NOTE_A4, 110}, {NOTE_B4, 110}, {NOTE_AS4, 90}, {NOTE_A4, 110},

  {NOTE_G4, 90},  {NOTE_E5, 90},  {NOTE_G5, 90},  {NOTE_A5, 120},
  {NOTE_F5, 90},  {NOTE_G5, 90},  {NOTE_REST, 90},
  ...
  // Parte B, más saltarina
  {NOTE_REST, 100}, {NOTE_G5, 90},  {NOTE_FS5, 90}, {NOTE_F5, 90},
  {NOTE_DS5, 110},  {NOTE_E5, 110}, {NOTE_REST, 80},
  {NOTE_GS4, 90},   {NOTE_A4, 90},  {NOTE_C5, 90},

  {NOTE_REST, 80},  {NOTE_A4, 90},  {NOTE_C5, 90},  {NOTE_D5, 90},
  {NOTE_REST, 100}, {NOTE_G5, 90},  {NOTE_FS5, 90}, {NOTE_F5, 90},
  {NOTE_DS5, 110},  {NOTE_E5, 110}, {NOTE_REST, 80},

  {NOTE_C6, 100}, {NOTE_C6, 100}, {NOTE_C6, 100}, {NOTE_REST, 80},
  {NOTE_G5, 100}, {NOTE_E5, 100}, {NOTE_C5, 100},
  {NOTE_A4, 100}, {NOTE_B4, 100}, {NOTE_C5, 180}, {NOTE_REST, 220}
};

// SFX cortos
static const MusicNote SFX_START[] = {
  { NOTE_C5, 70 }, { NOTE_E5, 70 }, { NOTE_G5, 90 }, { NOTE_C6, 120 }
};

static const MusicNote SFX_FLAP[] = {
  { NOTE_C6, 35 }, { NOTE_A5, 45 }
};

static const MusicNote SFX_SCORE[] = {
  { NOTE_E6, 55 }, { NOTE_G6, 80 }
};

static const MusicNote SFX_GAME_OVER[] = {
  { NOTE_C5, 90 }, { NOTE_G4, 100 }, { NOTE_E4, 120 }, { NOTE_C4, 180 }
};
...
static AudioMode audioMode = AUDIO_IDLE;

static bool musicActive = false;
static uint8_t musicIndex = 0;

static const MusicNote *activeSfx = nullptr;
static uint8_t activeSfxLen = 0;
static uint8_t activeSfxIndex = 0;

static uint32_t nextAudioEventMs = 0;
...
static void buzzerTone(uint16_t freq, uint8_t volume) {
  if (!ENABLE_BUZZER) {
    return;
  }
...
static void audioStartMusic() {
  if (!ENABLE_BUZZER || !ENABLE_MUSIC) {
    return;
  }
  ...
  if (audioMode != AUDIO_SFX) {
    audioMode = AUDIO_MUSIC;
    nextAudioEventMs = 0;
    noteOffMs = 0;
  }
}
...
static void audioStopMusic() {
  ...
  if (audioMode == AUDIO_MUSIC) {
    audioMode = AUDIO_IDLE;
    buzzerOff();
  }
}
...
static void playSfx(const MusicNote *sfx, uint8_t len) {
  if (!ENABLE_BUZZER || !ENABLE_SFX || sfx == nullptr || len == 0) {
    return;
  }
...
static void playSfxStart() {
  playSfx(SFX_START, ARRAY_LEN(SFX_START));
}

static void playSfxFlap() {
  playSfx(SFX_FLAP, ARRAY_LEN(SFX_FLAP));
}

static void playSfxScore() {
  playSfx(SFX_SCORE, ARRAY_LEN(SFX_SCORE));
}

static void playSfxGameOver() {
  playSfx(SFX_GAME_OVER, ARRAY_LEN(SFX_GAME_OVER));
}

static void serviceAudio(uint32_t now) {
  if (!ENABLE_BUZZER) {
    return;
  }

  if (noteIsOn && noteOffMs != 0 && (int32_t)(now - noteOffMs) >= 0) {
    buzzerOff();
    noteOffMs = 0;
  }

  if (nextAudioEventMs != 0 && (int32_t)(now - nextAudioEventMs) < 0) {
    return;
  }

  if (audioMode == AUDIO_SFX) {
    if (activeSfx == nullptr || activeSfxIndex >= activeSfxLen) {
      activeSfx = nullptr;
      activeSfxLen = 0;
      activeSfxIndex = 0;
    ...
    MusicNote n = activeSfx[activeSfxIndex++];

    uint16_t dur = n.durMs;
    uint16_t gate = (dur * 78) / 100;

    buzzerTone(n.freq, buzzerVolumeSfx);
    ...
    nextAudioEventMs = now + dur;
    return;
  }

  if (audioMode == AUDIO_MUSIC && musicActive) {
    MusicNote n = FLAPPY_THEME[musicIndex++];

    if (musicIndex >= ARRAY_LEN(FLAPPY_THEME)) {
      musicIndex = 0;
    }
    ...
    buzzerTone(n.freq, buzzerVolumeMusic);
...
static bool checkCollision() {
  ...
  for (uint8_t i = 0; i < 3; i++) {
    ...
    bool safelyInsideGap =
      ((birdTop + COLLISION_MARGIN) > gapTop) && ((birdBottom - COLLISION_MARGIN) < gapBottom);
