...
bool JW_MatrixButtons::beginDirect(const uint8_t *buttonPins,
                                   uint8_t buttonCount,
                                   bool invertLogic,
                                   uint32_t debounceMs,
                                   uint8_t inputMode)
{
  ...
  _directPins = buttonPins;
  _directCount = buttonCount;
  _btnCount = buttonCount;
  ...
  for (uint8_t i = 0; i < _directCount; i++)
  {
    pinMode(_directPins[i], _inputMode);
  }
...
void JW_MatrixButtons::updateMatrix()
{
...
void JW_MatrixButtons::updateDirect()
{
  if (!_directPins || _directCount == 0)
    return;

  lock();

  for (uint8_t id = 0; id < _directCount; id++)
  {
    bool raw = readDirectPin(_directPins[id]);
    debounceUpdate(_directDeb[id], raw);
    _btnStable[id] = _directDeb[id].stable;
  }
...
void JW_MatrixButtons::resetStates()
{
  ...
  {
    _directDeb[i].stable = false;
    _directDeb[i].lastRaw = false;
    _directDeb[i].lastChange = 0;
...
bool JW_MatrixButtons::readDirectPin(uint8_t pin) const
{
  int v = digitalRead(pin);
  return _invert ? (v == LOW) : (v == HIGH);
}
                                 ...
                                 bool snapToStepOnRepeat) const
{
  ...
  // Consumir presses pendientes de los dos botones.
  uint8_t decPress = 0;
  uint8_t incPress = 0;
  ...
  auto applyRepeatStep = [&](bool isInc, int16_t step) {
    ...
    if (isInc)
    {
      ...
      {
        base = (uint32_t)((v / (uint32_t)step) * (uint32_t)step);
      }
  ...
  // Si el usuario presiona ambos a la vez, se aplicará primero dec y luego inc.
  for (;;)
  {
    int16_t step = 0;
